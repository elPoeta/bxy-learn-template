class WizardNewVariableDefineForm extends WizardNewVariableForm {
  ...
  template() {
    ...
    const {
      ...
      arraySizeOne = "",
      arraySizeTwo = "",
    } = values ? values : this.dnd.block;
    return `<div class="wizard-box-edit-dialog" style="grid-template-rows: 30px auto;">
         <h4>Define variable</h4>
         <section id="declarationContainer" class="defineWizardContainer">
           <h4>Declaration</h4>
           <div id="declarationDropZone" class="varItemsDropZone" data-zone="declarationDropZone" data-content="Drop declaration here">${this.getDeclaration(
             declaration
           )}</div>
         </section>
         ${
           this.dnd.block.type !== "inputBlock"
             ? `<section id="typeContainer" class="defineWizardContainer">
              <h4>Type</h4>
              <div id="typeDropZone" class="varItemsDropZone" data-zone="typeDropZone" data-content="Drop type here">${this.getDeclarationType(
                declarationType
              )}</div>
            </section>`
             : ""
         ...
         ${
           this.dnd.block.type !== "inputBlock"
             ? `<section id="typeSizeContainer" class="defineWizardContainer ${
                 declarationType === "Normal" ? "hide" : ""
               }">
              <div>
                <h4>Size One</h4>
                <div id="typeSizeOneDropZone" class="handleTextClick varItemsDropZone defineSizeContainer" data-zone="sizeDropZone"  data-content="Drop size one here">${this.getSizeOne(
                  arraySizeOne,
                  declarationType
                )}</div>
              </div>
              <div id="typeSizeTwoZone" class="${
                declarationType !== "MultiDimensional Array" ? "hide" : ""
              }">
                <h4>Size Two</h4>
               <div id="typeSizeTwoDropZone" class="handleTextClick varItemsDropZone defineSizeContainer" data-zone="sizeDropZone" data-content="Drop size two here">${this.getSizeTwo(
                 arraySizeTwo,
                 declarationType
               )}</div>
              </div>  
         ...
         <section id="nameContainer" class="defineWizardContainer">
           <h4>Name</h4>
           <div id="nameDropZone" class="handleTextClick varItemsDropZone" data-zone="nameDropZone" data-content="Drop name here">${this.getVarName(
             variableName
           )}</div>
         </section>
         <section class="defineVarDragContainer custom-gray-scroll">
           ...
           <h4 class="${
             this.dnd.block.type === "inputBlock" ? "hide" : ""
           }">Type</h4>
           <div class="defineItemsContainer ${
             this.dnd.block.type === "inputBlock" ? "hide" : ""
           }">
            ...
            <h4>Values</h4>
           <div class="defineItemsContainer">
             <div class="draggable defineItems" style="background-color:#B2EBF2;" draggable="true" data-info='${JSON.stringify(
               { type: "nameDropZone", val: "text" }
             )}'>Name</div>
             <div class="draggable defineItems ${
               this.dnd.block.type === "inputBlock" ? "hide" : ""
             }" style="background-color:#B2EBF2;" draggable="true" data-info='${JSON.stringify(
      { type: "sizeDropZone", val: "size" }
    )}' >Size</div>
           </div>
         </section>
  ...
  declarationTemplate() {
    return this.declarations
      .map(
        (declaration) =>
          `<div class="draggable defineItems" style="background-color:#C8E6C9;" draggable="true" data-info='${JSON.stringify(
            { type: "declarationDropZone", val: declaration }
          )}'>${declaration}</div>`
      )
      .join("");
  }
  ...
  typesTemplate() {
    return this.variableTypes
      .map(
        (type) =>
          `<div class="draggable defineItems" style="background-color:#FFCC80;"  draggable="true" data-info='${JSON.stringify(
            { type: "typeDropZone", val: type }
          )}'>${type}</div>`
      )
      .join("");
  }
  ...
  getDeclaration(declaration) {
    return !Utils.isEmpty(declaration)
      ? `<h5 data-define='${JSON.stringify({
          val: declaration,
  ...
  getDeclarationType(declarationType) {
    return !Utils.isEmpty(declarationType)
      ? `<h5 data-define='${JSON.stringify({
          val: declarationType,
          id: "typeDropZone",
        })}'>${declarationType}</h5>`
  ...
  getSizeOne(arraySizeOne, declarationType) {
    return declarationType === "Array" ||
      declarationType === "MultiDimensional Array"
      ? `<h5 data-define='${JSON.stringify({
          val: arraySizeOne,
          id: "typeSizeOneDropZone",
        })}'>${arraySizeOne}</h5>`
      : "";
  }
  ...
  cloneTouchItem(target) {
    ...
    this.sourceInfoItem = JSON.parse(
      this.dragSourceItem.getAttribute("data-info")
    );
  ...
  handleMouseMove(ev) {
    if (!this.activeEvent) return;
    const rect = document
      .querySelector(".wizard-box-edit-dialog")
  ...
  isInTouchDropZone(dropZone) {
    const id =
      dropZone.dataset.zone !== "sizeDropZone"
        ? dropZone.id
  ...
  handleDropAction(target) {
    ...
    switch (id) {
      ...
      case "typeDropZone":
        if (id === this.sourceInfoItem.type) {
          target.innerHTML = `<h5 data-define='${JSON.stringify({
            val: this.sourceInfoItem.val,
            id,
          })}'>${this.sourceInfoItem.val}</h5>`;
          this.checkType(id);
        }
        break;
      default:
        if (id === this.sourceInfoItem.type) this.showTextForm(target);
    }
  }
  ...
  showTextForm(element) {
    ...
    this.textBtn.addEventListener(
      "click",
      this.handleTextForm.bind(this, element)
    );
  }
  ...
  handleTextForm(element, ev) {
    ...
    if (Utils.isEmpty(value)) {
      this.dnd.displayExpressionError(
        "newVarExpressionError",
        "Expression is empty"
      );
      return;
    }
    ...
    this.dnd.block.variableName = tempVarName;
    element.innerHTML = `<h5 data-define='${JSON.stringify({
      val: value,
      id: element.id,
    })}'>${value}</h5>`;
    this.textBtn.removeEventListener(
      "click",
      this.handleTextForm.bind(this, element)
    );
  ...
  templateTextForm(id) {
    const title =
      id === "nameDropZone"
        ...
        ? "Size One"
        : "Size Two";
    return `<div class="wizard-box-edit-dialog wizard-box-edit-dialog-text " style="border:3px solid #f3610a;">
         <h4>${title}</h4>
  ...
  updateWizardBox() {
    ...
    this.element.value = this.getExpression(
      this.getDataDefineObject(dataDefineItems, dataDefine)
    );
  ...
  getDataFromDndItems() {
    ...
    for (const item of items) {
      ...
      if (
        ["declarationDropZone", "typeDropZone", "nameDropZone"].includes(zone)
      ) {
        if (!children) {
          this.dnd.displayExpressionError(
      ...
      if (["typeSizeOneDropZone", "typeSizeTwoDropZone"].includes(zone)) {
        const index = items.findIndex(
          (el) => el.dataset.zone === "typeDropZone"
        );
        ...
        if (
          ("Array" === typeDropZoneVal && "typeSizeOneDropZone" === zone) ||
      ...
      if (children) dataDefine.push(JSON.parse(children.dataset.define));
    }
    return dataDefine;
  }
  ...
  handleClickNameZone(ev) {
    ev.preventDefault();
    const target =
      ev.target.tagName.toLowerCase() === "h5"
        ? ev.target.parentElement
        : ev.target;
  ...
  listenersManager(listenType) {
    ...
    this.overlayEdition[listenType](
      "mousedown",
    ...
    this.nameArraySizeDropZone.forEach((element) =>
      element[listenType]("click", this.handleClickNameZone.bind(this))
    );
  }
}
