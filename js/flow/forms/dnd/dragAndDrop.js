class DragAndDrop {
    constructor(options) {
      ...
      this.draggableElements = document.querySelectorAll(options.draggableSelector);
      this.dropZone = document.querySelector(options.dropZoneSelector);
      this.currentDropZone = document.querySelector(options.currentDropZoneSelector); 
      this.specialDropZones = options.specialDropZones;
      this.expressionBuildedVisible = document.querySelector(options.expressionSelectorVisible);
      this.expressionBuildedHidden = document.querySelector(options.expressionSelectorHidden);
      this.expressionBuildedHiddenToParse = document.querySelector('#expressionBuildedHiddenToParse');
      this.expressionContainer = document.querySelector(options.expressionContainer);
    ...
    addListeners() {
      ...
      if(this.block.type === 'inputBlock') {
        this.radioInputs = document.querySelectorAll('input[name="optionTypeWizard"]');
        this.radioInputs
         .forEach(radio => radio.addEventListener('change', this.handleInputRadioBtn.bind(this)));
      }
    }
    ...
    handleDragOver(ev) {
      ...
      if (target.classList.contains('dropZone')) {
        ...
        if(this.block.type !== 'forBlock' ) {
          const initialValue = [...this.currentDropZone.children].indexOf(target) + 1;
    ...
    handleDrop(ev) {
      ...
      if (!target.classList.contains('dropZone') && !target.classList.contains('draggable') && !target.classList.contains('wizardForDrop')) {
        ...
        if (this.dragSourceItem != target) {
          if(this.dragSourceItem.classList.contains('cloned')) { 
            if((type === 'defineBlock' || type === 'inputBlock') && this.currentDropZone.id !== 'valueVar') {
            	this.removeDndListeners();
              return;
            }
    ...
    dropAction(appendType, dataWe, target) {
      ...
      if(appendType === 'appendChild') {
        if((type === 'defineBlock' || type === 'inputBlock') &&
           this.currentDropZone.id !== 'valueVar') {
          if(!this.appendDefineAction(clone)) return;   
        }   
        if(type === 'forBlock' && !this.appendForAction(clone)) {
        	this.displayExpressionError('builderExpressionError', `this item can't be drop here`);  
        	return;
        }
    ...
    appendDefineAction(clone) {
      ...
      if((this.currentDropZone.id === 'defineVar' && dataWe.name !== 'variable') || 
    ...
    appendForAction(clone) {
    	...
    	if(this.currentDropZone.id === 'forAssign') {
    		if(!['assign', 'variable', 'number', 'wildcard', 'function'].includes(dataWe.name)) {
    			clone.remove();
    			return false;
    		}
    	}
    	...
    	if(this.currentDropZone.id === 'forUpdate') {
    		if(['text', 'equal', 'not-equal', 'greater-than',
    			'greater-than-equal', 'less-than', 'less-than-equal',
    			'and', 'or', 'not'].includes(dataWe.name)) {
    			clone.remove();
    			return false;
    		}
    	}
    ...
    handleMouseMove(ev) {
      if(!this.activeEvent) return;
      const rect = document.querySelector('.window-modal').getBoundingClientRect();
      ...
      this.specialDropZones.forEach(el => {
        ...
        if(this.validDropZone(box.getBoundingClientRect(), this.dragSourceItem)) {
      ...
      this.inDropZone(this.currentDropZone.getBoundingClientRect(), this.dragSourceItem, this.currentDropZone);
    }
    ...
    handleMouseUpAction() {
      ...
      if (index > -1) {
        ...
        this.updateTouchSourceItem(dataWe, (this.dragSourceItem.children.length < 2));
        this.currentDropZone.insertBefore(this.dragSourceItem, this.currentDropZone.children[index + 1]);
    ...
    getZoneElementIndex(children) {
      for (let i = 0; i < children.length; i++) {
        if (this.validDropZone(children[i].getBoundingClientRect(), this.dragSourceItem)) {
          return i;
        }
      }  
      return -1;
    ...
    updateTouchSourceItem(data, hasChildren) {
      ...
      if((type === 'input' && hasChildren) || (type === 'helper' && name === 'wildcard' && hasChildren)) {
        const fillColor = color.font;
        const svg = this.createSvgIcon(fillColor);
        svg.addEventListener('click', this.handleEdition.bind(this));
    ...
    getModifyTarget(target) {
      const { isNew, enable, ...rest } = JSON.parse(target.getAttribute('data-we'))
      ...
      case 'forBlock':
        if(element.parentElement.parentElement.id === 'forAssign') {
        	...
        	for(const child of children) {
        		...
        		if(dataWE.forVar && data.userValue !== dataWE.userValue) existVarAssign = true;
        	}	
        	if(!existVarAssign) {
    ...
    buildDefaultWizardExpression() {
      const children = Array.from(this.currentDropZone.children);
      this.expressionBuildedVisible.value = children.reduce((acc,curr) => acc + curr.firstElementChild.value + ' ','');
      this.expressionBuildedHidden.value = this.childrenReduce(children, false);
      this.expressionBuildedHiddenToParse.value = this.childrenReduce(children, true);
      this.evalWizardExpression();      
    }
    ...
    buildForWizardExpression() {
      ...
      dropZoneChildren.forEach((dropZoneChild,index) => {
        ...
        expression += children.reduce((acc,curr) => acc + curr.firstElementChild.value + ' ','');
        expression += index < 2 ? '; ' : '';
      });
    ...
    childrenReduce(children, hiddenReduce) {
      return children.reduce((acc,curr) => {
        ...
        const symbol = ['+', '-', '*', '/', '%', '=', '<', '>'].includes(data.value);
        const lastChar = acc.charAt(acc.length-1);
        const spaceStart = (!symbol && ['+', '-', '*', '/', '%', '=', '<', '>',].includes(lastChar) && data.userValue !== '=' ) ? ' ' : ''; 
        const spaceEnd = symbol ? '' : ' ';
        let userValue = isNaN(data.userValue) ? data.userValue.trim() : data.userValue;
        if(hiddenReduce && data.name === 'number') userValue = userValue.replace(/d*(f|L)/g, '');
    ...
    expressionContainerBorder(m) {
      const border = m === 'ok' ? '2px solid #2cb912' : m === 'error' ? '2px solid #b50c0c' : '2px solid #ccc';
    ...
    evalWizardExpression() {
      ...
      const code = this.block.type !== 'forBlock' ? this.getWizardExpressionToParse() : `for(${this.getWizardExpressionToParse()}){}`;
      ...
      for(let i = 0; i < len; i++) {
        if(i > 0 && i < len - 1) {
          if(elements[i].name === 'equal') {
            ...
            if((((l.name === 'variable' && l.extraProps.declaration == 'String') || l.name === 'text')) && 
            (r.name === 'text' || (r.name === 'variable' && r.extraProps.declaration == 'String'))) {
              let hiddenExpression = this.getWizardExpression().replace(`${l.userValue} == ${r.userValue}`, `${l.userValue}.equals(${r.userValue})`);
    ...
    getDeclaredVariables() {
      ...
      for(const element of this.getDroppedElements()) {
        if(element.name === 'variable') {
          code += `var ${element.userValue} = ${this.getValueByDeclaration(element.extraProps.declaration)};\n`;
        }
      }
      return code;  
    }
    ...
    changeInputZoneContent(action) {
      ...
      defineZone.dataset.content = action === 'add' ? 'select drop zone' : 'define drop zone';
      assignZone.dataset.content = action === 'add' ? 'disabled' : 'assign drop zone';
      valueZone.dataset.content = action === 'add' ? 'disabled' : 'value drop zone';
    ...
    isInputSelectVar() {
      ...
      const value = document.querySelector('input[name="optionTypeWizard"]:checked').value;
    ...
    removeDndListeners() {
      ...
      if(this.block.type === 'inputBlock') {
        this.radioInputs
         .forEach(radio => radio.removeEventListener('change', this.handleInputRadioBtn.bind(this)));
      }
      this.dragSourceItem.remove();
    }
