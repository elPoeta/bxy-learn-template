class DoWhileBlockJava extends DoWhileBlock {
    ...
    return {
      ...
      content:
        `<div style="font-size:1.4em">Variables</div>
         ...
         <input type="text" id="expression" name="expression" value='${this.code.replace(/'/g, "&apos;")}' required placeholder='count >= 10' style="width: 95%; padding:8px;" required >
  ...
  handlerWizardOk(div,ev) {
    ...
    if(typeof isLogicalExpression !== 'boolean') {
      this.wizardForm.dnd.expressionContainerBorder('error');
      this.wizardForm.dnd.displayExpressionError('builderExpressionError', 'Syntax error: expression should be logical');
      return;
    }
