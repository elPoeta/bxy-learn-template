class ForBlockJava extends ForBlock {
         ...
         <div id="selectContainer" class="${this.radioVar === 'defineVar' ? 'hide' : ''}">
           ...
           <select id="variables" name="variables" style="padding:8px; width: 100%;">
             ${!stateVariables.length ?
               ...
               `<option value='${variable.name}' ${this.variableName === variable.name ? 'selected' : ''}>${variable.declaration} ${variable.name} ${variable.declarationType} | ${variable.scope}</option>`
             )).join('')}`
            }
           </select>
  ...
  handlerSelectCombo(stateVariables) {
    ...
    selectVarCombo.addEventListener('change', e => {
      ...
      const stateVar = [...this.getAllVariableScope()].map(vs => vs[1]).filter(vs => vs.name === varSelected)[0];
  ...
  setNewExpression() {
    ...
    const operation = document.querySelector('#operation').value;
    const varCompareValue = document.querySelector('#varCompareValue').value;
    const declaration = document.querySelector('#defineVar').checked ? 'int ' : '';
  ...
  handlerWizardOk(div,ev) {
    ...
    const { variableName, initialValue, operator, compareValue, updateOperation } = evaluatedExpression.payload;
    const varState = [...this.getAllVariableScope()].map(vs => vs[1]).filter(vs => vs.id === this.id);
  ...
  evalForExpression() {
    ...
    if(this.isEmptyExpression(assignExp, testExp, updateExp)) return { succsess: false, message: 'Syntax error: has empty drop zone' };
  ...
  evalIndividualForExpression({ assignExp, testExp, updateExp }) {
    try{
      ...
      if(!codeParsed.length && (codeParsed.length > 0 && codeParsed[0].length < 1)) {
        return { succsess: false, message: `Unknown error` };
      }
      const props = codeParsed[0][1];
      if(!['>', '>=', '<', '<='].includes(props.radioOption)) {
        return { succsess: false, message: `Syntax error: test operator should be > >= < <=` };
      }
      const payload = {
      ...
      return { succsess: true, payload };     
    	  ...
    	  if(!this.wizardForm.dnd.validateForm) return this.notEvalForExpression({ assignExp, testExp, updateExp });  
  ...
  isEmptyExpression(assignExp, testExp, updateExp) {
    ...
    return Utils.isEmpty(assignExp) || Utils.isEmpty(testExp) || Utils.isEmpty(updateExp); 
  }
