class InputBlockJava extends InputBlock {
  ...
  templateContent() {
    ...
    return {
      ...
      content: `<div style="font-size:1.4em; margin-top:10px; margin-bottom:5px;">Option</div>
         <span style="font-size:1.5em; padding:4px;">Select variable</span><input type="radio" name="optionType" id="selectVar" ${
           this.radioOption === "selectVar" ? "checked" : ""
         }  value="selectVar" style="margin-right: 15px; padding:4px; position: relative; top: 0px;">
         <span style="font-size:1.5em; padding:4px;">Define new</span><input type="radio" name="optionType" id="defineMVar"  ${
           this.radioOption === "defineVar" ? "checked" : ""
         } value="defineVar" style="margin-right: 15px; padding:4px; position: relative; top: 0px;">
         <div id="selectContainer" class="${
           this.radioOption === "defineVar" ? "hide" : ""
         }">
          ...
          <div id="defineContainer" class="${
            this.radioOption === "selectVar" ? "hide" : ""
          }">
  ...
  handlerModalButtons(div) {
    document.querySelector("#ok-modal").addEventListener("click", (e) => {
      ...
      this.variableName =
        this.radioOption === "selectVar"
          ? formData.get("variables")
          : formData.get("variableName");
      ...
      if (this.radioOption === "selectVar") {
        const varState = [...this.getAllVariableScope()]
          .map((vs) => vs[1])
          .filter((vs) => vs.name === this.variableName);
        const declaration =
          varState.length > 0 ? varState[0].declaration : "String";
  ...
  dndDefineVarAction(div, hiddenValue) {
    ...
    const { variableName = "", declaration = "String" } =
