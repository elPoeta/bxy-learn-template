class OutputBlockJava extends OutputBlock {
  ...
  templateContent() {
    ...
    return {
      ...
      content:
        `<div style="font-size:1.4em; margin-top:10px; margin-bottom:5px;">Print option</div>
         <span style="font-size:1.5em; padding:4px;">print line</span><input type="radio" name="optionType" id="println" ${this.radioOption === 'println' ? 'checked' : ''}  value="println" style="margin-right: 15px; padding:4px; position: relative; top: 0px;">
         <span style="font-size:1.5em; padding:4px;">print</span><input type="radio" name="optionType" id="print"  ${this.radioOption === 'print' ? 'checked' : ''} value="print" style="margin-right: 15px; padding:4px; position: relative; top: 0px;">
         ...
         <input type="text" id="expression" name="expression" value='${this.expression.replace(/'/g, "&apos;")}' required placeholder='Ex: "Your name is: "+myVarName+"!"'  style="width: 95%; padding:8px;" required >
  ...
  handlerModalButtons(div) {
    document.querySelector('#ok-modal')
      .addEventListener('click', e => {
        ...
        const printTemplate = this.radioOption === 'println' ?
  ...
  handlerCombo() {
    ...
    combo.addEventListener('change', e => {
      if (e.target.value === '1') {
        ...
        document.querySelector('#expression').value = this.imports[this.importValue].value;
        this.isImport = true;
      }
    ...
    importCombo.addEventListener('change', e => {
      document.querySelector('#expression').value = '';
      document.querySelector('#expression').value = this.imports[e.target.value].value;
  ...
  handlerWizardOk(div,ev) {
    ...
    this.radioOption = document.querySelector('input[name="optionTypeWizard"]:checked').value;
    const printTemplate = this.radioOption === 'println' ?
            'System.out.println(' : 'System.out.print('
