class CodeBlockJava extends CodeBlock {
  constructor(id, canvas, x, y, w, h, type, code, scaleFactor, isProgram, languageOutput) {
    ...
    this.imports = [{ id: 1, value: 'import java.util.*;' }, { id: 2, value: 'import static java.lang.Math.*;' }, { id: 3, value: 'import java.util.Random;' }];
    this.importValue = 1;
    this.isImport = false;
  }
  ...
  templateContent() {
    ...
    return {
      title: 'code block',
      help:
        `<p>Choose Variables or Imports: change Variable list to imports list</p>
      ...
      content:
        `<div style="font-size:1.4em">Choose Variables or Imports</div>
        <select id="chooseList" name="chooseList" style="padding:8px; width: 100%;">
          <option value="1" ${this.isImport ? '' : 'selected'}>List of Variables</option>
          <option value="2" ${this.isImport ? 'selected' : ''}>List of Imports</option>
        </select>
        ...
        <select id="imports" name="imports" style="padding:8px; width: 100%;">
          ...
          `<option value='${imp.id}' ${this.importValue === imp.id ? 'selected' : ''}>${imp.value}</option>`
        )).join('')
        }
        </select>   
        </div>
  ...
  handlerCombo() {
    ...
    combo.addEventListener('change', e => {
      if (e.target.value === '1') {
        ...
        document.querySelector('#expression').value = this.imports[this.importValue - 1].value;
        this.isImport = true;
      }
    ...
    importCombo.addEventListener('change', e => {
      document.querySelector('#expression').value = '';
      document.querySelector('#expression').value = this.imports[e.target.value - 1].value;
