class DefineBlockJava extends DefineBlock {
  constructor(id, canvas, x, y, w, h, type, code, scaleFactor, isProgram, languageOutput) {
    ...
    this.reservedWords = ['scanner', 'Class', 'String', 'char', 'byte', 'short', 'int', 'float', 'double', 'boolean'];
  ...
  templateContent() {
    return {
      title: 'define',
      help:
        ` <p>Declaration: declaration type variable</p>
      ...
      content:
        `<div style="font-size:1.4em;">Declaration</div> 
          ...
          <div id="sizeContent" style="display:flex; width: 98%; ${this.declarationType === 'Array' ? 'justify-content: flex-start;' : 'justify-content: space-around;'}">
          <div style="width: 45%;" class="size1 ${this.declarationType === 'Normal' ? 'hide' : ''}">
            <div style="display:block; font-size:1.4em; margin-top:10px;">Array Size 1</div> 
            <input type="text" id='sizeSingle' name='sizeSingle' value='${this.arraySizeOne}' style="padding:8px; width: 95%;"/>
          </div>
          <div style="margin-left:10px; width: 45%;" class="size2 ${this.declarationType !== 'MultiDimensional Array' ? 'hide' : ''}">
            <div style="display:block; font-size:1.4em; margin-top:10px;">Array Size 2</div> 
            <input type="text" id='sizeMulti' name='sizeMulti' value='${this.arraySizeTwo}'  style="padding:8px; width: 95%;"/>
          </div>
          </div>
  ...
  handlerSelectCombo() {
    ...
    selectCombo.addEventListener('change', e => {
      ...
      if (e.target.value === 'Array') {
        ...
        document.querySelector('#sizeContent').style = 'width: 98%; display: flex; justify-content: flex-start;';
      ...
      if (e.target.value === 'MultiDimensional Array') {
        ...
        document.querySelector('#sizeContent').style = 'width: 98%; display: flex; justify-content: space-around;';
      }
    });
  }
  ...
  findAndUpdateInputsBlocks() {
    const varState = [...this.getAllVariableScope()].map(vs => vs[1]).filter(vs => vs.id === this.id);
    const inputBlocks = this.c.program.filter(block => block.type === 'inputBlock');
    ...
    inputBlocks.forEach(inputBlock => {
      if (inputBlock.radioOption === 'selectVar') {
        ...
        if (varStateObj && varStateObj.id === inputBlock.externalVariableId) {
          if (varStateObj.declarationType === 'Normal' && varStateObj.declaration !== inputBlock.declaration) {
            const index = this.c.program.findIndex(blk => blk.id === inputBlock.id);
            this.c.program[index].declaration = varStateObj.declaration;
            this.c.program[index].code = this.c.program[index].getInputCode(varStateObj.declaration);
  ...
  updateWizardState(index, declaration) {
    ...
    this.c.program[index].droppedElements[0][0].extraProps.declaration = declaration;
  }
  ...
  handlerWizardOk(div, ev) {
    ...
    const varState = [...this.getAllVariableScope()].map(vs => vs[1]).filter(vs => vs.id === this.id);
    const { variableName = '', declaration = 'String', declarationType = 'Normal', arraySizeOne = 1, arraySizeTwo = 1 } = defineExp.length > 0 ? defineExp[0].extraProps : {};
