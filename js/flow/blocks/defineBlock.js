class DefineBlock extends IOBlock {
  constructor(
    ...
    this.twelve = 12 / 1.5;
    this.twenty = 20 / 1.5;
    this.twentyEight = 28 / 1.5;
    this.twentyTwo = 22 / 1.5;
    this.six = 6 / 1.5;
    this.ten = 10 / 1.5;
    this.thirtyFour = 34 / 1.5;
    this.eighteen = 18 / 1.5;
  ...
  render() {
    ...
    const { blockColor, fontColor, lineColor } = !this.colors
      ? configEditor.flow.customizedBlocks.defineBlock
      : this.colors;
    ...
    this.c.ctx.beginPath();
    this.c.ctx.moveTo(
      this.x + this.w - this.eleven,
      this.y + this.h - this.twentyEight
    );
    this.c.ctx.lineTo(
      this.x + this.w + this.eighteen + 1,
      this.y + this.h - this.twentyEight
    );
    this.c.ctx.stroke();
    this.c.ctx.beginPath();
    this.c.ctx.moveTo(
      this.x + this.w - this.eleven + 2,
      this.y + this.h - this.twentyEight
    );
    this.c.ctx.lineTo(
      this.x + this.w - this.eleven + 2,
    ...
    this.c.ctx.lineTo(
      this.x + this.w + this.four,
      this.y + this.h - this.twentyTwo + 4
    );
    this.c.ctx.stroke();
    this.c.ctx.beginPath();
    this.c.ctx.moveTo(
    ...
    this.c.ctx.lineTo(
      this.x + this.w + this.six,
      this.y + this.h - this.thirtyFour - 4
    );
    this.c.ctx.stroke();
    ...
    if (!this.editing) {
      ...
      const { code, textW } = this.renderTextCode({
        maxWidth: this.w - this.twenty,
      });
      this.c.ctx.fillText(
        code,
        textW,
