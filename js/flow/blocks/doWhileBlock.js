class DoWhileBlock extends Block {
    constructor(id, canvas, x, y, w, h, type, code, scaleFactor, isProgram, languageOutput) {
        ...
        this.ellipseW = w / 2;
        this.ellipseH = h / 2;
        this.blockProps = { w: 240, h: 210, fullH: 210, defaultLongBranch: 60, accumH: this.doWhileDefaultAccum, branch: 'none', widthLine: 60 };
        ...
        this.incrementRingLine = 60;
        this.innerOutLineHeight = 60;
        this.incrementRightLine = 60;
        this.incrementLeftLine = 60;
        this.incrementWidthLeftLine = 60;
        this.plusRing = 10;
        this.plusHookW = 60;
        this.plusHookH = 20;
        this.yhtext = 15;
        ...
        this.hooks = isProgram ? [
            new Hook(canvas, x + w / 2, y + 10 - this.hookLineHeight + this.incrementRingLine + 20, w, h, false, "ring", '', 'in', type),
            new Hook(canvas, x + w / 2, y + 10 - this.outLineHeight + this.incrementOutLine - h / 2, w, h, false, 'hook', 'bottom', 'inner-out-yes', type),
            new Hook(canvas, x + w, y + h / 2, w, h, false, 'hook', 'right', 'join', type),
            new Hook(canvas, x, y + h / 2, w, h, false, 'hook', 'left', 'join', type, this.incrementLeftLine),
            new Hook(canvas, x + w / 2, y, w, h, true, "ring", '', 'in-inner', type),
            new Hook(canvas, x + w / 2, y + h, w, h, false, 'hook', 'bottom', 'out', type)
        ] : [];
    ...
    render() {
        ...
        const { blockColor, fontColor, lineColor } = !this.colors ? 
                configEditor.flow.customizedBlocks.doWhileBlock : this.colors;
        ...
        if ((this.x + this.w > 200) && (!this.grabed)) {
          ...
          this.c.ctx.fillText(LANGUAGE_FLOW[CURRENT_LANG].yes, this.x - this.hookLineHeight / 2, this.y + this.h / 2 - this.yhtext);
          this.c.ctx.fillText(LANGUAGE_FLOW[CURRENT_LANG].no, this.x + this.w + this.c.ctx.measureText(LANGUAGE_FLOW[CURRENT_LANG].no).width + this.wLang, this.y + this.h / 2 + this.yhtext);
          if (this.compileError.hasError) {
            const width = this.c.ctx.measureText('Error !!').width -  this.errorTxt;
            this.drawErrorText('Error !!', this.fontSizeCode, this.x + this.w + width, this.y + this.h);
          } 
        }
    ...
    updateHooksMove(x, y) {
      ...
      this.hooks[0].y = y + this.plusRing - this.incrementRingLine * 2 + this.h / 2;
      this.hooks[0].w = this.w;
      this.hooks[0].h = this.h;

      this.hooks[1].x = x + this.w / 2;
      this.hooks[1].y = y - this.innerOutLineHeight - this.plusHookW + this.h / 2 + this.plusHookH;
    ...
    updateHeight(props) {
        ...
        this.incrementLeftLine += isRemoved ? height / 2 : height > 0 ? height / 2 : 0;
        this.incrementRingLine += isRemoved ? height / 2 : height > 0 ? height / 2 : 0;
        this.innerOutLineHeight += isRemoved ? height : height > 0 ? height : 0;
        if (this.hooks[0].occupied && !this.isLocked)
          this.y += height;
    }
    ...
    updateWidth(branchWidth) {
      ...
      this.lastDimension.w = Math.abs(this.incrementWidthLeftLine) + Math.abs(this.incrementWidthRightLine) + this.w;
