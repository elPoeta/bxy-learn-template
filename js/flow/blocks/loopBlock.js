class LoopBlock extends Block {
  constructor(id, canvas, x, y, w, h, type, code, scaleFactor, isProgram, languageOutput) {
    ...
    this.blockProps = { w: 240, h: 200, fullH: 200, defaultLongBranch: 60, branch: 'none', widthLine: 60 };
    ...
    this.hooks = isProgram ? [
      new Hook(canvas, x + w / 2, y, w, h, false, "ring", '', 'in', 'loopBlock'),
      new Hook(canvas, x + w / 2, y + h, w, h, false, 'hook', 'bottom', 'inner-out-yes', 'loopBlock'),
      new Hook(canvas, x + w, y + h / 2, w, h, false, 'hook', 'right', 'join', 'loopBlock'),
      new Hook(canvas, x, y + h / 2, w, h, false, 'hook', 'left', 'join', 'loopBlock'),
      new Hook(canvas, x - h, y + h * 3 - h / 2 + 10, w, h, false, 'hook', 'bottom', 'out', 'loopBlock'),
      new Hook(canvas, x + w / 2, y + h * 3 + 10, w, h, true, "ring", '', 'end', 'loopBlock')
    ] : [];
    this.cogwheel = isProgram ? new CogwheelBlock(canvas, x, y, w, h, 25, 10, id) : null;  
  }
  ...
  render() {
    ...
    this.c.ctx.quadraticCurveTo(this.x + this.w, this.y, this.x + this.w, this.y + this.r);
    this.c.ctx.lineTo(this.x + this.w, this.y + this.h - this.r);
    this.c.ctx.quadraticCurveTo(this.x + this.w, this.y + this.h, this.x + this.w - this.r, this.y + this.h);
    this.c.ctx.lineTo(this.x + this.r, this.y + this.h);
    this.c.ctx.quadraticCurveTo(this.x, this.y + this.h, this.x, this.y + this.h - this.r);
    ...
    if ((this.x + this.w > 200) && (!this.grabed)) {
      ...
      this.c.ctx.fillText(LANGUAGE_FLOW[CURRENT_LANG].yes, this.x + this.w / 2 + this.xwText, this.y + this.h + this.yhtext);
      this.c.ctx.fillText(LANGUAGE_FLOW[CURRENT_LANG].no, this.x - this.xwText, this.y + this.h / 2 + this.yhtext);
      this.c.ctx.fillText(LANGUAGE_FLOW[CURRENT_LANG].yes, this.x + this.w + this.c.ctx.measureText(LANGUAGE_FLOW[CURRENT_LANG].yes).width + this.wLang, this.y + this.h / 2 + this.yhtext);
      if (this.compileError.hasError) {
        const width = this.c.ctx.measureText('Error !!').width - this.errorTxt;
        this.drawErrorText('Error !!', this.fontSizeCode, this.x + this.w + width, this.y + this.h + this.errorTxt);
      }
    }
  ...
  updateHooksMove(x, y) {
    ...
    this.hooks[4].y = y + this.outLineHeight + this.incrementOutLine + this.plusHook;
    this.hooks[4].w = this.w;
    this.hooks[4].h = this.h;

    this.hooks[5].x = x + this.w / 2;
    this.hooks[5].y = y + this.hookLineHeight + this.incrementBottomRing + this.plusHook;
  ...
  resize() {
    ...
    if (this.isProgram && !this.grabed && !this.isLocked) {
      if (this.c.expanded && this.hooks[0].occupied) {
        ...
        this.blockProps.w = this.w + 120;
        this.lastDimension.w = this.widthBeforeExpand + this.blockProps.w - 240;
  ...
  updateWidth(branchWidth) {
    ...
    this.lastDimension.w = Math.abs(this.incrementWidthLeftLine) + Math.abs(this.incrementWidthRightLine) + this.blockProps.w;
    this.widthBeforeExpand = this.lastDimension.w;
  }
  
  edit() {
    document.getElementById("text-box").style.display = "block";
    document.getElementById("text-box").style.top = this.y + this.c.yScroll + this.c.yOffset - this.h / 2 + 85 + "px";
