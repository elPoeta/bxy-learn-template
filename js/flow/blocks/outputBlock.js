class OutputBlock extends IOBlock {
  ...
  render() {
    ...
    const { blockColor, fontColor, lineColor } = !this.colors
      ? configEditor.flow.customizedBlocks.outputBlock
      : this.colors;
    ...
    this.c.ctx.beginPath();
    this.c.ctx.moveTo(
      this.x + this.w - this.sixteen,
      this.y + this.h - this.five
    );
    this.c.ctx.lineTo(
    ...
    this.c.ctx.lineTo(
      this.x + this.w - this.one,
      this.y + this.h - this.ten - 1
    );
    this.c.ctx.stroke();
    ...
    if (!this.editing) {
      ...
      const { code, textW } = this.renderTextCode({
        maxWidth: this.w - this.twenty - 2,
      });
      this.c.ctx.fillText(
        code,
        textW,
        this.y + this.h / this.textProps.verticalAlignText,
        this.w - this.twenty - 2
      );
