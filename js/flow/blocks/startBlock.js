class StartBlock extends ContainerBlock {
  constructor(id, canvas, x, y, w, h, type, code, scaleFactor, isProgram, languageOutput) {
    ...
    this.blockProps = { w: 120, h: 130, fullH: 130, defaultLongBranch: 0  };
    this.outHookIndex = { out: 0, innerOutYes: 0, innerOutNo: 0 };
    this.hooks = [
      new Hook(canvas, x + w / 2, y + h, w, h, true, 'hook', 'bottom', 'out', type)
    ];
    this.projectCompositeId = null;
    this.fullCode = '';
    this.globalScope = new BlockState();
    this.importsAndInstancesState = new BlockState();
    this.cogwheel = (canvas.tabs.length > 0 && canvas.tabs[canvas.selectedTab].type === 'f_body' )  ? 
            new CogwheelBlock(canvas, x, y, w, h, 20, 10, id) : null;
    this.editable = canvas.tabs.length > 0 && canvas.tabs[canvas.selectedTab].type === 'f_body'; 
    this.yhtext = 15;
    this.wLang = 5;
  ...
  renderTextCode({ horizontalText, maxWidth }) {
    const textW = this.editable ? this.x + this.textProps.startText : this.x + this.w / 2;
    if (!this.editable || Utils.isEmpty(this.code) || typeof this.code !== 'string')
