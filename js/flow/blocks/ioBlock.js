class IOBlock extends Block {
  ...
  resize() {
    ...
    if (this.isProgram && !this.grabed && !this.isLocked) {
      if (this.c.expanded && this.hooks[0].occupied) {
        ...
        const w = !Utils.isEmpty(this.code) ? this.c.ctx.measureText(this.code).width + plus : this.w;
  ...
  edit() {
    ...
    document.getElementById("text-box").style.top = this.y + this.c.canvas.yScroll + this.c.canvas.yOffset - this.h / 2 + 85 + "px";
    document.getElementById("text-box").style.left = this.x + this.c.canvas.xScroll + "px";
  ...
  setImportStatement(statementValue) {
    const importVarState = this.c.program[0].importsAndInstancesState.get('import');
    const value = !importVarState ? new Set([statementValue]) : new Set([...importVarState.value, statementValue]);
    this.c.program[0].importsAndInstancesState.update('import', { name: 'import', value, declarationType: 'import' });
  }

  setInstanceStatement(statementValue) {
    const { name, value, declaration, declarationType, used } = statementValue;
    this.c.program[0].importsAndInstancesState.update(name, { name, value, declaration, declarationType, used });
  }

  delImportStatement(importDef) {
    const importState = this.c.program[0].importsAndInstancesState.get('import');
    if (!importState) return;
    const arr = [...importState.value].filter(val => val !== importDef);
    if (!arr.length) {
      this.c.program[0].importsAndInstancesState.remove('import');
    } else {
      this.c.program[0].importsAndInstancesState.update('import', { name: 'import', value: new Set(arr), declarationType: 'import' });
    }
  }
