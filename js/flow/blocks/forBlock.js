class ForBlock extends LoopBlock {
  ...
  setDroppedElementsBase() {
    ...
    this.droppedElements = [
      [variableItem, assignItem, numberItem],
  ...
  getForWizardItems() {
    const wizardItems = !this.c.wizardItems.length
      ? WizardItems.getDefaultItems()
      : this.c.wizardItems[0].items;
    const items = [];
    wizardItems.forEach((item) => {
      item.items.forEach((subItem) => {
        if (
          ["variable", "number", "assign", "addition", "less-than"].includes(
            subItem.name
          )
        ) {
          const { type, name, displayValue, value, color, pos, userValue } =
            subItem;
          items.push({
            type,
            name,
            ...
            color,
            pos,
            userValue: name === "variable" ? "EDIT_ME" : userValue,
          });
        }
      });
    });
