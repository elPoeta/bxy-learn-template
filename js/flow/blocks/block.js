class Block {
  ...
  addBreakpoint() {
    ...
    this.breakpoint = new Breakpoint(this.c, this.x + this.w / 2, this.y + this.h, this.id);
  ...
  setRenderColor({ blockColor, lineColor }) {
    if (this.c.expanded && !this.isProgram ) {
      const strokeColor = (this.type !== 'startBlock' && this.type !== 'endBlock') ? '#858585' : configEditor.flow.customizedBlocks[this.type].lineColor;
      const fillColor = (this.type !== 'startBlock' && this.type !== 'endBlock') ? '#dddddd' : configEditor.flow.customizedBlocks[this.type].blockColor; 
      this.c.ctx.strokeStyle = strokeColor;
      this.c.ctx.fillStyle = fillColor;
    } else {
        this.c.ctx.strokeStyle = this.isIntercepted ? "#46ea4b" :
        this.error ? "red" : (this.selected || this.highlighted) ? "blue" : lineColor;    
  ...
  renderTextCode({ horizontalText, maxWidth }) {
    ...
    if (this.type === 'ifBlock') {
      ...
      textW = this.isProgram ? this.x + this.textProps.startText : this.x + this.w / 2;
    }
    if (!this.isProgram || Utils.isEmpty(this.code) || typeof this.code !== 'string')
  ...
  genericTemplate(props) {
    const style = "height: 335px; top: -130px; width: max-content; padding: 15px 10px; text-align:left;";
    return (
      `<div id="window-dim-code" class="window-dim-code-default">
          <div id="window-code" class="window-modal custom-gray-scroll">
            <h2 style="width:125px;font-size:1.4rem;position:absolute;top:20px;right:20px;color:#777;text-shadow: 2px 2px #bbbbbb;text-align:end;">${props.title}</h2> 
            ...
            <section id="manualBuilder"> 
              <div id="title" style="font-weight: bold; display: flex; justify-content:end; align-items: flex-end;">
                <div class="filter-tooltip"><i style="font-size: 1.1em; color: #fff; background: #1B3F91; width: 1em; display: block; border-radius: 50%; text-align: center;">?</i>
                  <span class="filter-tooltiptext filter-tooltip-left" style='${props.styles ? props.styles : style}'>
                    ${props.help}
                  </span>
                </div>  
              </div>
            ...
            <section id="wizardBuilder" class="hide" style="width:inherit;max-height:100vh;">
              ...
              <div style="display: flex;align-items: end;width: inherit;">
                ${this.templateSwitch()}
                ${this.templateButtons('buttonsWizard', 'ok-modal-wizard', 'cancel-modal-wizard')}
              </div>
            </section>
          </div>
       </div>`
  ...
  renderWizardBuilder(div) {
    ...
    const items = !this.c.wizardItems.length ? WizardItems.getAllItems() : this.c.wizardItems;
  ...
  templateSwitch() {
    return (
      `<section style="display: flex;align-items: center;margin-right: auto;font-size: 1.2em;">
  ...
  addBuilderFormListeners(div) {
    this.switchBuildedExpression = document.querySelector('#viewRealBuildedExpression');
    this.switchValidateExpression = document.querySelector('#validateExpressionSwitch');
    ...
    this.changeWizardGrid = document.querySelector('#changeWizardGridContainer');
    this.builderTabs.addEventListener('click', this.switchBuilderTab.bind(this));
    this.cancelBuilderBtn.addEventListener('click', this.closeBuilder.bind(this,div));
    this.switchBuildedExpression.addEventListener('change', this.handleSwitchExpression.bind(this));
    this.switchValidateExpression.addEventListener('change', this.handleSwitchValidateExpression.bind(this));
    this.changeWizardGrid.addEventListener('click', this.handleChangeWizardGrid.bind(this));
  }
  ...
  switchBuilderTab(ev) {
    ...
    if(target.id === 'manualBuilderTab') {
    	this.updateWizardGridStyles(document.querySelector('#changeWizardGridBottom'));
      this.toggleBuilderClass(target, 'manualBuilder', 'wizardBuilder');
        ...
        this.updateWizardGridStyles(document.querySelector('#changeWizardGridLeft'));
    }
  }
  ...
  toggleBuilderClass(target, sectionActive, sectionHide) {
    ...
    document.querySelector(`#${sectionHide}Tab`).classList.remove('builder-tab-active');
    document.querySelector(`#${sectionHide}Tab`).classList.add('builder-tab-inactive');
    document.querySelector(`#${sectionHide}`).classList.add('hide');
    document.querySelector(`#${sectionActive}`).classList.remove('hide');
    const borderDel = sectionActive === 'wizardBuilder' ? 'border-builder-clear-m' : 'border-builder-clear-w';
    const borderAdd = sectionActive === 'wizardBuilder' ? 'border-builder-clear-w' : 'border-builder-clear-m';
    const className = sectionActive === 'wizardBuilder' ? 'window-dim-code-wizard-left-right' : '';
    document.querySelector('#window-dim-code').className = '';
    document.querySelector('#window-dim-code').className = [`window-dim-code-default ${className}`];
  ...
  handleChangeWizardGrid(ev) {
  	ev.preventDefault();
    const target = ev.target;
    if(!target.classList.contains('change-wizard-grid') || 
    		target.classList.contains('change-wizard-grid-active')) return;
    this.updateWizardGridStyles(target);
  }
  
  updateWizardGridStyles(target){
    [...this.changeWizardGrid.children]
      .forEach(el => el.classList.remove('change-wizard-grid-active'));
 	  ...
 	  const dragZoneBuilderItems = document.querySelector('.dragZone-builderItems');
 	  const wizardExpressionsContainer = document.querySelector('.wizard-expressions-container');
    ...
    switch(target.id) {
     	case 'changeWizardGridLeft':
     		windowDimCode.className = ['window-dim-code-default window-dim-code-wizard-left-right'];
     	  document.querySelector('#wizardBuilder').style.height = `${h}px`; 
     	  wizardAllZones.className = ['wizard-all-zones wizard-all-zones-left-right'];
     	  dragZoneBuilderItems.className = ['dragZone-builderItems dragZone-builderItems-left custom-gray-scroll'];
     	  wizardExpressionsContainer.className = ['wizard-expressions-container wizard-expressions-container-left-right'];
     	  dropZoneContainer.className = ['dropZone-container dropZone-container-left'];
     	  document.querySelector('#dropZone').style.height = `${document.querySelector('#window-code').getBoundingClientRect().height - 295}px`;
   		  dragZoneBuilderItems.style.height = `${document.querySelector('#window-code').getBoundingClientRect().height - 295}px`;
   		  break;
   	  case 'changeWizardGridRight':
   	  	windowDimCode.className = ['window-dim-code-default window-dim-code-wizard-left-right'];
       	document.querySelector('#wizardBuilder').style.height = `${h}px`; 
     	  dropZoneContainer.className = ['dropZone-container dropZone-container-right'];
     	  dragZoneBuilderItems.className = ['dragZone-builderItems dragZone-builderItems-right custom-gray-scroll'];
     	  wizardExpressionsContainer.className = ['wizard-expressions-container wizard-expressions-container-left-right'];
     	  wizardAllZones.className = ['wizard-all-zones wizard-all-zones-left-right'];
       	document.querySelector('#dropZone').style.height = `${document.querySelector('#window-code').getBoundingClientRect().height - 295}px`;
   	   	dragZoneBuilderItems.style.height = `${document.querySelector('#window-code').getBoundingClientRect().height - 295}px`;
   	  ...
   	  case 'changeWizardGridBottom':
   	  	windowDimCode.className = ['window-dim-code-default window-dim-code-wizard-bottom'];
     	  document.querySelector('#wizardBuilder').style.height = `auto`; 
     	  dropZoneContainer.className = ['dropZone-container'];
     	  dragZoneBuilderItems.className = ['dragZone-builderItems custom-gray-scroll'];
  ...
  changeStyleOfInnerZones(id) {
    ...
    switch(id) {
     	case 'changeWizardGridLeft':
      case 'changeWizardGridRight':	
      	document.querySelectorAll('.wizardForDropTitle')
      	  .forEach(el => el.classList.remove('hide'));
      	document.querySelectorAll('.wizardForDrop')
  ...
  removeBuilderListener() {
    this.builderTabs.removeEventListener('click', this.switchBuilderTab.bind(this));  
    this.cancelBuilderBtn.removeEventListener('click', this.closeBuilder.bind(this));  
  }
  ...
  defineNewTemplate(selector, message) {
    return (
      `<section class="wizardWrapFunc-container" style="gap:30px;padding:20px;width:300px;">
         <div>
  ...
  addDefineNewListeners(selector) {
    this.defineNewModalBtn = document.querySelector(`#defineNewModalBtn${selector}`);
    this.defineNewModalBtn.addEventListener('click', this.handleDefineModalButtons.bind(this));
  }
  ...
  handleDefineModalButtons(ev) {
    ...
    if(id === 'defineModalOk') {
      ...
      this.declareNewFunctionAction();
        ...
        this.defineNewModalBtn.removeEventListener('click', this.handleDefineModalButtons.bind(this));   
        document.querySelector('body').classList.remove('backdropOver');
        this.overlayDefineNew.remove();
    }
  }
  ...
  declareNewFunctionAction() {
    ...
    this.c.program.push(BlockFactory.getImplementation(flowChartEditor.uuid(), this.c, 'wrapBlock', 0, 0, w, h, '', true, this.c.languageOutput, 'function_declaration'));
    ...
    this.c.update();
    const wizardForm = WrapFormFactory.getImplementation(this.c, null, 'new_empty', '[]', indexRing, []);
  ...
  isInvalidNameInScope() {
    ...
    if (!this.parentId) {
      if (['inputBlock', 'defineBlock'].includes(this.type)) {
        if (this.type === 'inputBlock' && this.radioOption === 'selectVar') return;
        const isUsed = this.c.program.some(block => block.type === 'forBlock' 
          && block.parentId === null && block.radioVar === 'defineVar' 
    ...
    if (this.parentId) {
      ...
      const isInvalidInParent = this.findVarNameInScope(this.parentsScope, this.blockProps.branch);
      if (isInvalidInParent) return true;
      const children = this.c.getChildren(this.c.program[index].id)
  ...
  findVarNameInScope(blocks, branch) {
    for (let i = 0; i < blocks.length; i++) {
      ...
      if (['startBlock', 'whileBlock', 'forBlock', 'doWhileBlock', 'ifBlock'].includes(type)) {
  ...
  hasVarIntoInnerBlock(allDescendants) {
    for (let i = 0; i < allDescendants.length; i++) {
      ...
      if (this.c.program[idx][scopeProp].hasKey(this.variableName) &&
        (['defineBlock', 'inputBlock'].includes(this.type))) {
        return true;
      }
      if (this.c.program[idx].type === 'ifBlock') {
        if (this.c.program[idx].ifScopeBranchRight.hasKey(this.variableName) &&
          (['defineBlock', 'inputBlock'].includes(this.type))) {
          return true;
        }
      }
  ...
  setScope(varState) {
    ...
    if (!this.parentId) {
      const index = this.type !== 'forBlock' ? 0 : this.c.getBlockIndex(this.id);
      const { type } = this.c.program[index];
      const { scopeProp, scope } = this.getScopeProps(type, this.blockProps.branch);
      this.checkRenameVar(varState, index, scopeProp);
      this.addVariableToScope({ index, scopeProp, scope, isInFlow });
    } else {
      const index = this.type !== 'forBlock' ? this.c.getBlockIndex(this.parentId) : this.c.getBlockIndex(this.id);
      const { type } = this.c.program[index];
      const { scopeProp, scope } = this.getScopeProps(type, this.blockProps.branch);
      this.checkRenameVar(varState, index, scopeProp);
      this.addVariableToScope({ index, scopeProp, scope, isInFlow });
  ...
  getAllVariableScope() {
    ...
    if (this.parentId) {
      ...
      this.allParentScope.forEach(block => {
        ...
        const { scopeProp } = this.getScopeProps(type, branch);
        if (['startBlock', 'whileBlock', 'forBlock', 'doWhileBlock', 'ifBlock'].includes(type)) {
          allScopes = block[scopeProp].size() > 0 ? [...allScopes, block[scopeProp].state] : allScopes;
          branch = block.blockProps.branch;
        }
      });
    }
  ...
  getScopeProps(type, branch) {
    switch (type) {
      ...
      case 'doWhileBlock':
        return { scopeProp: 'doWhileScope', scope: this.c.scopeType.DO_WHILE_SCOPE };
      case 'forBlock':
        return { scopeProp: 'forScope', scope: this.c.scopeType.FOR_SCOPE };
      case 'ifBlock':
        if (branch === 'NO') {
          return { scopeProp: 'ifScopeBranchLeft', scope: this.c.scopeType.IF_SCOPE_LEFT };
        }
        return { scopeProp: 'ifScopeBranchRight', scope: this.c.scopeType.IF_SCOPE_RIGHT };
      default:
        return { scopeProp: 'globalScope', scope: this.c.scopeType.GLOBAL_SCOPE };
    }
  }
  ...
  getFixedX() {
    ...
    if (!state.hasOwnProperty('indexArrowHook')) throw new Error("index hook property not found");
    if (!state.hasOwnProperty('lastHook')) throw new Error("last hook property not found");
    ...
    const hookX = this.c.program[index].hooks[indexArrowHook].x > 0 ? this.c.program[index].hooks[indexArrowHook].x : -1 * this.c.program[index].hooks[indexArrowHook].x;
    return this.c.program[index].hooks[indexArrowHook].x > 0 ?
            Math.abs(this.type !== 'ifBlock' ? Math.abs(this.w / 2 - hookX) : hookX) :
            -1 * Math.abs(this.type !== 'ifBlock' ? Math.abs(this.w / 2 + hookX) : hookX);
  }
