class IfBlock extends Block {
  constructor(
    ...
    this.branchDimension = { r: 120 / 1.5, l: 120 / 1.5 };
    this.widthBeforeExpand = 240;
    this.grabPosition = { x: 0, y: h / 2 };
    this.hookLineWidth = 60 / 1.5;
    ...
    this.hooks = isProgram
      ? [
          ...
          new Hook(
            canvas,
            this.x - this.w - this.outLineHeight + this.incrementOutLine,
            this.y + this.h,
            w,
            h,
            false,
          ...
          new Hook(
            canvas,
            this.x - this.w - this.hookLineHeight,
            this.y + 10 + h + this.hookLineHeight,
            w,
            h,
            true,
            "ring",
          ...
          new Hook(
            canvas,
            this.x - this.w - this.hookLineHeight,
            this.y + 20 + h + this.hookLineHeight,
            w,
            h,
            false,
            "hook",
          ...
          new Hook(
            canvas,
            x,
            this.y + 20 + h + this.outLineHeight + this.incrementOutLine,
            w,
            h,
            false,
            "hook",
    ...
    this.cogwheel = isProgram
      ? new CogwheelBlock(canvas, x, y, w, h, 5, 25, id)
      : null;
  ...
  render() {
    ...
    const { blockColor, fontColor, lineColor } = !this.colors
      ? configEditor.flow.customizedBlocks.ifBlock
      : this.colors;
    ...
    if (!this.editing) {
      ...
      const { textW, code } = this.renderTextCode({
        horizontalText: this.textProps.horizontalAlignText,
        maxWidth: this.w * 1.8,
      });
      this.c.ctx.fillText(
        code,
        textW,
        this.y + this.h / 2 + this.textProps.verticalAlignText,
        this.w * 1.8
      );
    }
    ...
    if (this.isProgram && !this.grabed && !this.isLocked) {
      if (this.c.expanded && this.hooks[0].occupied) {
        const w = !Utils.isEmpty(this.code)
          ? this.c.ctx.measureText(this.code).width
          : this.w;
        this.w = !Utils.isEmpty(this.code)
          ? w < this.w + 60 / 1.5
            ? this.w
            : w - 60 / 1.5
          : this.w;
        ...
        this.lastDimension.w =
          this.widthBeforeExpand +
          this.w * 2 +
          this.hookLineWidth * 2 -
          240 / 1.5;
  ...
  updateHooksMove(x, y) {
    ...
    this.hooks[3].x =
      x - this.w - this.hookLineHeight + this.incrementWidthLeftLine;
    this.hooks[3].y = y + this.outLineHeight;
    this.hooks[3].w = this.w;
    this.hooks[3].h = this.h;
    this.hooks[4].x =
      x + this.w + this.hookLineHeight + this.incrementWidthRightLine;
    ...
    this.hooks[5].x =
      x - this.w - this.hookLineHeight + this.incrementWidthLeftLine;
    this.hooks[5].y =
      y + this.plusRing + this.h + this.hookLineHeight + this.incrementLeftRing;
    ...
    this.hooks[6].y =
      ...
      this.h +
      this.hookLineHeight +
      this.incrementRightRing;
    this.hooks[6].w = this.w;
    this.hooks[6].h = this.h;
    this.hooks[7].x =
      x - this.w - this.hookLineHeight + this.incrementWidthLeftLine;
    this.hooks[7].y =
      y + this.plusBottom + this.h + this.hookLineHeight + this.heightLeftLine;
    ...
    this.hooks[8].x =
      x + this.w + this.hookLineHeight + this.incrementWidthRightLine;
    this.hooks[8].y =
      y + this.plusBottom + this.h + this.hookLineHeight + this.heightRightLine;
  ...
  setAccumulativeHeight() {
    this.accumulativeHeight =
      this.leftBranchHeight >= this.rightBranchHeight
        ? this.leftBranchHeight
        : this.rightBranchHeight;
  ...
  updateWidth(branchWidthYes, branchWidthNo, plusIfWYes, plusIfWNo) {
    ...
    this.incrementWidthLeftLine =
      branchWidthNo > 0 ? -1 * branchWidthNo : branchWidthNo;
    this.branchDimension = {
      r:
        this.incrementWidthRightLine + this.w + this.hookLineWidth + plusIfWYes,
      l:
  ...
  edit() {
    ...
    document.getElementById("text-box").style.top =
      this.y + this.h / 2 - 9 + this.c.yScroll + this.c.yOffset + "px";
