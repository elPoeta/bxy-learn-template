class PaletteManager {
  ...
  constructor(canvas) {
    this.c = canvas;
    this.jsonPalette = { io: ['define', 'code', 'input', 'output'], loop: ['while', 'doWhile', 'for'], decision: ['if'], wrap: ['wrap'] };
    this.gap = 20;
    this.validKeys = ['io', 'loop', 'decision', 'wrap'];
    this.validBlocks = ['define', 'code', 'input', 'output', 'while', 'doWhile', 'for', 'if', 'wrap'];
      ...
      if (this.validKeys.includes(key)) {
        ...
        value.forEach(val => {
          if (this.validBlocks.includes(val)) {
            ...
            this.palette.push(BlockFactory.getImplementation(flowChartEditor.uuid(), this.c, `${val}Block`, x, this.yPaletteBlockAndText, w, h, LANGUAGE_FLOW[CURRENT_LANG][`${val}Block`], false, ''));
            const plusH = val !== 'if' ? h : h * 2;
            this.yPaletteBlockAndText = this.yPaletteBlockAndText + plusH + this.gap;
          }
        });
      } 
    }
  ...
  createTextPalette(key) {
    ...
    return new TextPalette(this.c, 0, this.yPaletteBlockAndText, 200, 15, LANGUAGE_FLOW[CURRENT_LANG][text], text, 1);
  }
