class ExpandShrink {
  ...
  expand() {
    ...
    if (node === this.c.endBlockId && this.c.tabs[this.c.selectedTab].type !== 'f_body') {
      this.c.setExpanded();
      return;
    }
  ...
  setNewWidth(descendant) {
    ...
    let plusIfWNo = 0
    if (type === 'ifBlock') {
      const { branchYes, branchNo, plusWYes, plusWNo } = this.setNewIfWidth(children, parentIndex);
    ...
    this.c.program[parentIndex].updateWidth(branchWidthYes, branchWidthNo, plusIfWYes, plusIfWNo);
    this.c.program[parentIndex].move(x, y);
  }

  setNewLoopWidth(children, parentIndex) {
    const { type, w, incrementWidthLeftLine, blockProps: { defaultLongBranch }} = this.c.program[parentIndex];
    const lineAdd = 30;
    ...
    if (maxType === 'ifBlock') {
      ...
      if (!childrenIf.length) {
        return type === 'doWhileBlock' ? lineAdd + incrementWidthLeftLine - defaultLongBranch : lineAdd + incrementWidthLeftLine;
      }
      const { maxWidth: maxWYes, type: maxTypeYes, idx: idxYes } = this.getMaxWidth(childrenIf.filter(child => child.props.branch === 'YES'))
      const { maxWidth: maxWNo, type: maxTypeNo, idx: idxNo } = this.getMaxWidth(childrenIf.filter(child => child.props.branch === 'NO'));
      if (maxWYes >= maxWNo) {
        const childHalfW = this.c.program[idx].branchDimension.r;
        const parentHalfW = type === 'doWhileBlock' ? incrementWidthLeftLine + defaultLongBranch : w / 2 + incrementWidthLeftLine + defaultLongBranch;
        ...
        if (childHalfW > parentHalfW) {
          ...
          return type === 'doWhileBlock' ? (sub <= lineAdd && sub >= 0) ? lineAdd : incrementWidthLeftLine - defaultLongBranch : (sub <= lineAdd && sub >= 0) ? lineAdd : incrementWidthLeftLine;
        } else {
          return type === 'doWhileBlock' ? lineAdd + incrementWidthLeftLine - defaultLongBranch : lineAdd + incrementWidthLeftLine;
        }
      } else {
  ...
  setNewIfWidth(children, parentIndex) {
    ...
    const { maxWidth: maxWYes, type: maxTypeYes, idx: idxYes } = this.getMaxWidth(children.filter(child => child.props.branch === 'YES'));
    const { maxWidth: maxWNo, type: maxTypeNo, idx: idxNo } = this.getMaxWidth(children.filter(child => child.props.branch === 'NO'));
    let branchYes = 0;
    let branchNo = 0;
    let plusWYes = 0;
    let plusWNo = 0;
  ...
  getMaxWidth(children) {
    ...
    children.forEach(child => {
      ...
      const { maxWidth: maxW, type: maxType } = this.calculateMaxWidth({ maxWidth, index });
      if (maxW > maxWidth) {
        maxWidth = maxW;
        type = maxType;
        idx = index;
  ...
  calculateMaxWidth({ maxWidth, index }) {
    const { lastDimension: { w: lastDimension }, type } = this.c.program[index];
  ...
  getAllDescendants(node, children, includeAll = false) {
    ...
    const getDescendants = children => {
      for (let i = 0; i < children.length; i++) {
        let childs = !includeAll ? this.c.getChildren(children[i].id)
