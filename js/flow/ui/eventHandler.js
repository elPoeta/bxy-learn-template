class EventHandler {
  constructor(canvas) {
    this.c = canvas;
    this.setupValues();
    this.ioBlocks = ['codeBlock', 'defineBlock', 'inputBlock', 'outputBlock', 'wrapBlock'];
    this.loopBlocks = ['whileBlock', 'forBlock'];
    this.blockStatement = ['whileBlock', 'forBlock', 'doWhileBlock', 'ifBlock'];
    this.allOuts = ['out', 'inner-out-yes', 'bottom-inner-out-yes', 'bottom-inner-out-no', 'inner-out-no'];
    this.outs = ['inner-out-yes', 'bottom-inner-out-yes', 'bottom-inner-out-no', 'inner-out-no'];
    ...
    this.c.canvas.addEventListener('mousemove', this.handlerMouseMove.bind(this));
    this.c.canvas.addEventListener('mousedown', this.handlerMouseDown.bind(this));
    ...
    this.c.canvas.addEventListener('contextmenu', this.showContextMenu.bind(this));
    this.c.canvas.addEventListener('wheel', this.handleWheel.bind(this));
    this.c.canvas.addEventListener('touchstart', this.handleTouchStart.bind(this), false);
    this.c.canvas.addEventListener('touchmove', this.handleTouchMove.bind(this), false);
    this.c.canvas.addEventListener('touchend', this.handleTouchEnd.bind(this), false);
    this.dragConsoleBar.forEach(element => {
      ...
      element.addEventListener('mouseleave', this.handlerConsoleLeave.bind(this));
      element.addEventListener('touchstart', this.handleConsoleTouchStart.bind(this), false);
      element.addEventListener('touchmove', this.handleConsoleTouchMove.bind(this), false);
      element.addEventListener('touchend', this.handleConsoleTouchEnd.bind(this), false);
    });
  ...
  handlerConsoleMove(ev) {
    ev.target.style.cursor = ev.target.getAttribute('data-dragg') === 'vertical' ? "ew-resize" : "ns-resize";
    this.c.updateCanvas();
  }
  ...
  createVerticalGhost() {
    ...
    const actualHeight = +document.querySelector(`#${this.c.IDCanvasContainer}`).offsetHeight;
    div.style.height = `${actualHeight - 45}px`;
    document.querySelector(`#${this.c.IDCanvasContainer}`).insertBefore(div, document.querySelector(`#${this.c.IDCanvasContainer}`).childNodes[0]);
  ...
  createHorizontalGhost() {
    ...
    div.style.top = `${+document.querySelector('#console-box').offsetTop  + 25}px`; 
  ...
  handleTouchStart(ev) {
    ...
    if (ev.touches.length > 1) {
      ...
      this.fingerDistance = this.getFingerDistance(ev);
        ...
        this.isPinch = false;
        Utils.dispatchMouseEvent({ touch, type: 'mousedown', element: this.c.canvas });
    }
  }
  ...
  handleTouchMove(ev) {
    ...
    if (ev.touches.length > 1) {
      this.dispatchZoom({ ev, touch });
        ...
        Utils.dispatchMouseEvent({ touch, type: 'mousemove', element: this.c.canvas }); 
    }
  }
      ...
      this.openContextMenuOnTouchEnd(); 
        ...
        if (this.isPinch) {
          this.updateMouseUp();
            ...
            Utils.dispatchMouseEvent({ touch: this.lastTouch, type: 'mouseup', element: this.c.canvas });
       }
     }
  ...
  handleConsoleTouchStart(ev) {
    ...
    Utils.dispatchMouseEvent({ touch, type: 'mousedown', element: window.document });
  ...
  handleConsoleTouchMove(ev) {
    ...
    Utils.dispatchMouseEvent({ touch, type: 'mousemove', element: window.document });
  }
  
  handleConsoleTouchEnd(ev) {
    if(!this.lastTouch) return;
    this.handlerConsoleUp(ev);
    Utils.dispatchMouseEvent({ touch: this.lastTouch, type: 'mouseup', element: window.document });
  }
  ...
  openContextMenuOnTouchEnd() {
    if(!document.querySelector('#containerMenuContext')) {
      setTimeout(() => {
        Utils.dispatchMouseEvent({ touch: this.lastTouch, type: 'contextmenu', element: this.c.canvas });
      },300);  
    } else {
        this.checkContextMenu(); 
    }  
  ...
  handlerMouseDown(ev) {
    ...
    setTimeout(() => {
      ...
      this.mouseY = ev.clientY - this.c.ctx.canvas.offsetTop - this.c.yOffset + 6 - 40;
      this.selectBlock(ev);
  ...
  closeTrash() {
    this.trashLid.style = 'transform: rotate(0deg); margin-bottom: 0px; background: #ff2323bf;';
    this.trashLidCap.style = 'transform: rotate(0deg); margin-bottom: 0px; background: #ff2323bf;';
  ...
  openTrash() {
    this.trashLid.style = 'transform: rotate(-25deg); margin-bottom: 7.5px; background: #fff;';
    this.trashLidCap.style = 'transform: rotate(-25deg); margin-bottom: 7.5px; background: #fff;';
  ...
  handleWheel(ev) {
    ...
    this.unSelectBlocks();
    const { x, y } = ev;
    const direction = ev.wheelDelta ? ev.wheelDelta > 0 ? 1 : -1 : ev.deltaY < 0 ? 1 : -1;
    if (ev.clientX > 200) {
      this.zoom({ x, y, direction });
    } else {
  ...
  zoom({ x, y, direction }) {
    ...
    const wx = (x - this.c.ctx.canvas.offsetLeft - this.c.xOffset - this.c.xScroll) / (this.c.canvas.width * this.c.scale);
    const wy = (y - this.c.ctx.canvas.offsetTop - this.c.yOffset - this.c.yScroll + 6 - 40) / (this.c.canvas.height * this.c.scale);
  ...
  scrollPalleteWheel({ direction }) {
    ...
    const diffH = paletteY - (this.c.canvas.height - document.querySelector('#canvasToolContainer').getBoundingClientRect().height) + paletteH;
    this.c.yScrollPalette += ((this.c.yScrollPalette + dy) > 0 || (this.c.yScrollPalette + dy) < -diffH) ? 0 : dy;
    this.c.updateCanvas();
  }
  
  screenToWorld(x, y) {
    x = (x - this.c.ctx.canvas.offsetLeft - this.c.xOffset - this.c.xScroll) / this.c.scale;
    y = (y - this.c.ctx.canvas.offsetTop - this.c.yOffset - this.c.yScroll + 6 - 40) / this.c.scale;
    return { x, y };
  }

  worldToScreen(x, y) {
    x = parseInt(x * this.c.scale + this.c.xScroll + this.c.xOffset + this.c.ctx.canvas.offsetLeft);
    y = parseInt(y * this.c.scale + this.c.yScroll + this.c.yOffset + this.c.ctx.canvas.offsetTop - 6 + 40); 
    return { x, y };
  }
  ...
  selectBlockEdited({ x, y }) {
    return this.c.program.map(program => {
      if (this.c.grabSplit || program.isLocked || this.c.selectBlock) return program;
      if (program.editing) {
  ...
  selectBlockFromPallete({ ev, x, y }) {
    const yPalette = (ev.clientY - this.c.ctx.canvas.offsetTop - this.c.yOffset - this.c.yScrollPalette + 6 - 40);
      ...
      if (!this.paletteGrabbed) {
        if (block.isTouching(this.mouseX, yPalette)) {
          ...
          if (this.c.tabs[this.c.selectedTab].id !== '1') {
            this.c.createNewProgram(type, x - this.c.xScroll, y - this.c.yScroll, w, h, '', 'function_calling');
            const index = this.c.program.length - 1;
            this.c.program[index].grabed = true;
            this.c.program[index].selected = true;
          } else {
            if (type === 'wrapBlock') {
              this.c.createNewProgram(type, x - this.c.xScroll, y - this.c.yScroll, w, h, '', 'function_declaration');
              const index = this.c.program.length - 1;
              this.c.program[index].grabed = true;
              this.c.program[index].selected = true;
            }
  ...
  selectWorkspace(ev) {
    ...
    ev.preventDefault();
    ev.stopPropagation();
    this.startX = parseInt(ev.clientX - this.c.ctx.canvas.offsetLeft - this.c.xOffset);
    this.startY = parseInt(ev.clientY - this.c.ctx.canvas.offsetTop - this.c.yOffset + 6 - 40);
    if(this.c.expanded) return;
    this.c.program = this.c.program.map(program => {
      if (program.isTouching(x, y) || this.mouseX < 200 || this.ctrlKeyPressed) {
        if (this.mouseX < 200) {
          this.c.palette.forEach(block => {
    ...
    this.c.palette.forEach(block => {
      ...
      if (!this.paletteGrabbed && ev.clientX <= 200) {
        if (block.isTouching(this.mouseX, yPalette)) {
          ...
          if ((ev.clientX > 160 && ev.clientX <= 200) || (ev.clientX > 0 && ev.clientX <= 40)) {
  ...
  getBlockScalePosition(type, index) {
    ...
    if (type === 'ifBlock') {
      mouseX = zoomX === 0 ?
        this.mouseX : (this.c.program[index].x - this.c.program[index].w / 2);
      mouseY = zoomX === 0 ?
        this.mouseY : (this.c.program[index].y + this.c.program[index].h);
    } else {
      mouseX = zoomX === 0 ?
        this.mouseX : (this.c.program[index].x + this.c.program[index].w / 2);
      mouseY = zoomX === 0 ?
        this.mouseY : (this.c.program[index].y + this.c.program[index].h / 2);
    }
  ...
  updateWorkspaceMove(ev) {
    if (this.c.workspaceGrabed) {
      ev.preventDefault();
      ev.stopPropagation();
      const mouseX = parseInt(ev.clientX - this.c.ctx.canvas.offsetLeft - this.c.xOffset);
      const mouseY = parseInt(ev.clientY - this.c.ctx.canvas.offsetTop - this.c.yOffset + 6 - 40);
  ...
  showCodeToolTip(ev) {
    ...
    this.c.program.forEach(program => {
      ...
      if (program.isTouching(x,y)) {
        if (!program.grabed && (!Utils.isEmpty(program.code) || program.compileError.hasError)) {
          ...
          const fix = this.c.ctxTip.measureText("H").actualBoundingBoxDescent / 2;
  ...
  getCodeTip(program) {
    if (program.type === 'startBlock' && this.c.tabs[this.c.selectedTab].id != '0' && this.c.tabs[this.c.selectedTab].id != '1') {
    ...
    return !program.compileError.hasError ?
      ...
      program.compileError.errorMessages
    ...
    this.c.program = this.c.program.map((program, i) => {
      if (program.grabed) {
          ...
          if (j != i) {
            ...
            programHook.hooks.forEach(hook => {
              if (this.allOuts.includes(hook.ioType)) {
                ...
                if (!programHook.isLocked && 
                     hook.isLineInterceptCircle(c, r) &&
                     programHook.hooks[0].occupied) {
                  okHook = true;
                  hookIndex = j;
                  program.x = this.setXOnDrop(program, programHook, hook.ioType);
                  this.moveInnerBlocks(program);
                  this.c.interceptedProgram = { programRing: program, programHook, arrowType: hook.ioType }
                }
              }
            });
          }
        ...
        if (ok) {
          ...
          this.setInterceptedInnerBlocksColorState({ intercepted: 'isIntercepted', isIntercepted: false, error: 'error', isError: false, lastGoodPosition: 'lastGoodPosition' });
        } else {
          this.c.canvas.style.cursor = 'not-allowed';
          program.error = true;
          this.setInterceptedInnerBlocksColorState({ intercepted: 'isIntercepted', isIntercepted: false, error: 'error', isError: true });
        }
        if (okHook) {
          this.c.canvas.style.cursor = 'grabbing';
          program.error = false;
          ...
          this.setInterceptedInnerBlocksColorState({ intercepted: 'isIntercepted', isIntercepted: true, error: 'error', isError: false });

        } else {
          program.isIntercepted = false;
          if (this.lastHookIndex > -1) this.c.program[this.lastHookIndex].isIntercepted = false;
          this.setInterceptedInnerBlocksColorState({ intercepted: 'isIntercepted', isIntercepted: false, error: null });
  ...
  moveInnerBlocks(program) {
    ...
    this.innerBlocks.forEach(innerBlock => {
      const idx = this.c.program.findIndex(block => block.id === innerBlock.id);
      if (idx < 0) return;
      const { id, parentId, type, blockProps: { branch }, accumulativeHeight } = this.c.program[idx];
      if (parentBlock.id !== parentId) {
        ...
        const indexParent = this.c.program.findIndex(block => block.id === parentId);
      ...
      if (program.id !== id) {
        if (parentBlock.type === 'doWhileBlock') {
          ...
          if (branch === 'NO') {
            accumNo += lastBlock && lastBlock.type === 'doWhileBlock' ? -lastBlock.accumulativeHeight :
              type === 'doWhileBlock' ? accumulativeHeight : 0;
            this.c.program[idx].move(x, parentBlock.y + accumNo);
            accumNo += this.c.program[idx].blockProps.fullH;
          } else {
            accumYes += lastBlock && lastBlock.type === 'doWhileBlock' ? -lastBlock.accumulativeHeight :
              type === 'doWhileBlock' ? accumulativeHeight : 0;
  ...
  getParentXPosition(parentBlock, type, branch) {
    ...
    if (type === 'ifBlock') {
      if (parentBlock.type === 'ifBlock') {
        if (branch === 'NO') {
          x = parentBlock.x - parentBlock.w - this.widthLine + parentBlock.hooks[1].incrementWidthLeftLine;
        } else {
          x = parentBlock.x + parentBlock.w + this.widthLine + parentBlock.hooks[2].incrementWidthRightLine;
        }
      } else {
        x += this.widthLine;
      }
      ...
      if (branch === 'NO') {
        x = parentBlock.x - parentBlock.w - (this.widthLine * 2) + parentBlock.hooks[1].incrementWidthLeftLine;
      } else {
        x = parentBlock.x + this.widthLine + parentBlock.hooks[2].incrementWidthRightLine;
      }
    }
    return x;
  }
  ...
  moveDoWhileBlocks(parentTemp, doWhileBlocks, accum) {
    if (doWhileBlocks.length > 0) {
      doWhileBlocks.forEach(blockIndex => {
        const { type, blockProps: { branch, fullH }, accumulativeHeight } = this.c.program[blockIndex];
        const fixAccum = this.getFixedInnerDoWhileYPosition(type, accumulativeHeight);
  ...
  getFixedInnerDoWhileYPosition(type, accumulativeHeight) {
    return this.loopBlocks.includes(type) ?
      ...
      type === 'ifBlock' ? (this.c.arrowDistance * 2) + accumulativeHeight : 0;
  }
  
  setInterceptedInnerBlocksColorState({ intercepted, isIntercepted, error, isError, lastGoodPosition }) {
    this.innerBlocks.forEach(innerBlock => {
      const idx = this.c.program.findIndex(block => block.id === innerBlock.id);
      this.c.program[idx][intercepted] = isIntercepted;
      this.c.program[idx][error] = error ? isError : false;
      this.c.program[idx][lastGoodPosition] = lastGoodPosition ? { x: innerBlock.x, y: innerBlock.y } : this.c.program[idx][lastGoodPosition];
    });
  }
    ...
    if (this.c.tabs[this.c.selectedTab].id != '1') {
      if (prgHookType === 'ifBlock') {
        if (ioType === 'out') {
          ...
          newBlockX += type !== 'ifBlock' ? w + hooks[2].incrementWidthRightLine : w + hooks[2].incrementWidthRightLine + this.widthLine;
        } else if (ioType === 'bottom-inner-out-yes') {
          newBlockX -= type !== 'ifBlock' ? this.widthLine : 0;
        } else if (ioType === 'inner-out-no') {
          newBlockX -= type !== 'ifBlock' ? w - hooks[1].incrementWidthLeftLine + (this.widthLine * 2) : w - hooks[1].incrementWidthLeftLine + this.widthLine;
  ...
  async dropBlock() {
    ...
    for (let i = 0; i < this.c.program.length; i++) {
      ...
      if (this.c.program[i].error && this.c.program[i].grabed) {
        this.c.program[i].move(this.c.program[i].lastGoodPosition.x, this.c.program[i].lastGoodPosition.y);
      ...
      if (this.mouseX < 200) {
        if ((this.c.program[i].type !== "endBlock") && (this.c.program[i].type !== "startBlock")) {
          if (grabed) {
            const { deletedBlock, lessI, undo } = await this.runDialogConfirm(this.blockBeforeDrop, i);
      ...
      for (let i = 0; i < this.c.program.length; i++) {
        if (this.c.program[i].selected) {
          ...
          if (!this.c.program[i].editing) {
            if ((this.c.program[i].type !== "startBlock") && (this.c.program[i].type !== "endBlock")) {
              const { deletedBlock, lessI, undo } = await this.runDialogConfirm(this.blockBeforeDrop, i);
              isDeleted = deletedBlock;
              undoDeleted = undo;
              if (isDeleted) {
  ...
  dropInterceptedProgram(undoDeleted) {
    ...
    if (this.c.interceptedProgram) {
      ...
      const { isUsed, isRehook, variableName } = this.isUsedVaribleOnSwap({ programRing, programHook, arrowType });
      if (isUsed) {
        if (!isRehook) {
          Dialog.noticeDialog({ title: "Error", text: `<span class="prj-name color-red">Swapped blocks contain same define variable in scope with name: <span style="color: #000; font-weight: bold; font-style: italic;">${variableName}</span></span>`, isExamSubmission: false });
        }
        this.resetValues();
        this.c.updateCanvas();
        this.flowErrorManager.execute();
  ...
  addNodeEdgesToInnerIf() {
    ...
    if (length >= 1) {
      if (['inner-out-yes'].includes(arrowType)) {
        const edge = nodeEdges[0];
        this.c.graph.removeEdge(programHook.id, edge);
        let node1 = nodeEdges[1] && nodeEdges[1] !== 'empty' ? nodeEdges[1] : null;
        ...
        let idx = this.c.program.findIndex(block => block.id === programRing.id);
        ...
        let node2 = nodeEdges[2] && nodeEdges[2] !== 'empty' ? nodeEdges[2] : null;
        if (nodeEdges[1]) {
          this.c.graph.removeEdge(programHook.id, node1);
        }
        if (nodeEdges[2]) {
  ...
  addNewNodeAndEdge() {
    ...
    this.c.graph.addNode(programRing.id);
    this.c.graph.addEdge(programHook.id, programRing.id);
    const idx = this.c.program.findIndex(block => block.id === programRing.id);
    this.updateStateHook(idx, programHook.id, arrowType);
  }
  ...
  replaceAndAddNewEdge(edgeId) {
    ...
    if (!nodeEdges || !nodeEdges.length) {
      ...
      if (['inner-out-yes'].includes(arrowType)) {
      ...
      } else if (['bottom-inner-out-yes', 'bottom-inner-out-no'].includes(arrowType)) {
        this.c.graph.addEdge(programHook.id, programRing.id);
        this.c.graph.addEdge(programRing.id, edge);
        let idx = this.c.program.findIndex(block => block.id === programRing.id);
        this.updateStateHook(idx, programHook.id, arrowType);
        ...
        if (tempEdge) {
          ...
          const idx = this.c.program.findIndex(block => block.id === tempEdge);
          this.updateStateHook(idx, programHook.id, 'inner-out-yes');
        }
        if (tempEdge2) {
          this.c.graph.addEdge(programHook.id, tempEdge2);
          const idx = this.c.program.findIndex(block => block.id === tempEdge2);
  ...
  interceptOutArrow() {
    const { programRing, programHook, arrowType } = this.c.interceptedProgram;
    const { length, firstNode, secondNode } = this.swapBlocks({ programRing, programHook, arrowType });
    this.addNodeEdgesToOut();
    const type = programRing.type
    const heightToAdd = type !== 'doWhileBlock' ? this.addToHeight[type] : 
      this.addToHeight[type] + programRing.accumulativeHeight;
    const indexRing = this.c.program.findIndex(block => block.id === programRing.id);
    ...
    this.setMissingEdgesOnSwapBlock({ length, firstNode, secondNode, programRing });
  ...
  loopBlockTypeInnerYes() {
    ...
    const indexRing = this.c.program.findIndex(block => programRing.id === block.id);
    const type = programRing.type;
    const indexArrowHook = programHook.outHookIndex.innerOutYes;
    const indexArrowHookRing = programRing.outHookIndex.out;
    this.c.program[indexRing].hooks[indexArrowHookRing].ioType = 'bottom-inner-out-yes';
    this.c.program[indexRing].blockProps.branch = 'YES';
    this.c.program[indexRing].parentId = programHook.id;
    const heightToAdd = type === 'doWhileBlock' ? programHook.y + programRing.accumulativeHeight : programHook.y;
  ...
  doWhileBlockTypeInnerYes() {
    ...
    this.c.program[indexRing].blockProps.branch = 'YES';
    this.c.program[indexRing].parentId = programHook.id;
    const heightToAdd = type === 'doWhileBlock' ?
      programHook.y + programRing.accumulativeHeight - programHook.accumulativeHeight + programHook.blockProps.accumH :
      programHook.y - programHook.accumulativeHeight + programHook.blockProps.accumH;
  ...
  ifBlockTypeInnerNo() {
    ...
    const indexArrowHookRing = programRing.outHookIndex.out;
    this.c.program[indexRing].hooks[indexArrowHookRing].ioType = 'bottom-inner-out-no';
  ...
  updateStateOnSwap() {
    const tree = this.c.graph.breadthFirstSearch(this.c.startBlockId).filter(g => g !== this.c.endBlockId);
    tree.forEach(node => {
      ...
      if (index > -1) {
        ...
        const indexArrowHook = flowChartEditor.API.getIndexHook(this.c.program[indexHook].type, state.arrowHook);
  ...
  isUsedVaribleOnSwap({ programRing, programHook, arrowType }) {
    ...
    if (['defineBlock', 'inputBlock', 'forBlock'].includes(programRing.type)) {  
      ...
      if (this.checkIoBlockScope({ programRing, programHook, arrowType }))
        return { isUsed: true, isRehook: false, variableName: programRing.variableName };
    }
    const { isParentVarUsed, parentVarName } = this.checkParentBlockScope(programHook, arrowType);
    if (isParentVarUsed)
      return { isUsed: true, isRehook: false, variableName: parentVarName };
    if (programRing.hooks[0].occupied) {
      this.removeAndUpdateVariablesScopeOnSwap({ programRing, programHook, arrowType });
    } else {
  ...
  removeScopeFromFlow() {
    ...
    if (['defineBlock', 'inputBlock'].includes(dropped.type)) {
      if (dropped.type === 'inputBlock' && dropped.radioOption === 'selectVar') return;
      if (!dropped.parentId) {
        if (this.c.program[0].globalScope.hasKey(dropped.variableName)) {
          this.c.program[0].globalScope.remove(dropped.variableName);
        }
      } else {
        const index = this.c.program.findIndex(block => block.id === dropped.parentId);
        const { scopeProp } = this.c.program[index].getScopeProps(this.c.program[index].type, dropped.blockProps.branch);
  ...
  reHook({ programRing, programHook, arrowType }) {
    ...
    if (lastRingState && (lastRingState.lastHook === programHook.id) &&
      (lastRingState.arrowHook === arrowType) && lastRingState.render) {
  ...
  isNotDefineVar(block) {
    if (block.type === 'inputBlock' && block.radioOption === 'selectVar') return true;
    if (!this.innerBlocks.length && block.type === 'forBlock' && block.radioVar === 'selectVar') return true;
    return false;
  }
  ...
  checkIoBlockScope({ programRing, programHook, arrowType }) {
    if (['inner-out-yes', 'inner-out-no'].includes(arrowType)) {
      ...
      const indexParent = this.c.program.findIndex(parentBlock => parentBlock.id === programHook.parentId);
      return this.blockHookHasVar(this.c.program[indexParent], programRing, arrowType);
    } else {
  ...
  blockHookHasVar(block, programRing, arrowType) {
    const branch = ['inner-out-no', 'bottom-inner-out-no'].includes(arrowType) ? 'NO' : 'YES';
    const { scopeProp } = block.getScopeProps(block.type, branch);
    if (!Utils.isEmpty(programRing.code) && 
      ...
      const children = this.c.getChildren(block.id)
        .filter(child => ['whileBlock', 'doWhileBlock', 'forBlock', 'ifBlock'].includes(child.type));
      const allDescendants = this.c.getAllDescendants(children);
      for (let i = 0; i < allDescendants.length; i++) {
        const idx = this.c.program.findIndex(child => child.id === allDescendants[i].id);
        const { scopeProp } = this.c.program[idx].getScopeProps(this.c.program[idx].type, 'NO');
        if (!Utils.isEmpty(programRing.code) && 
              this.c.program[idx][scopeProp].hasKey(programRing.variableName)) {
          const value = this.c.program[idx][scopeProp].get(programRing.variableName);
          if (['defineBlock', 'inputBlock'].includes(this.c.interceptedProgram.programRing.type)
        ...
        if (this.c.program[idx].type === 'ifBlock') {
          if (!Utils.isEmpty(programRing.code) && 
               this.c.program[idx].ifScopeBranchRight.hasKey(programRing.variableName)) {
            const value = this.c.program[idx].ifScopeBranchRight.get(programRing.variableName);
            if (['defineBlock', 'inputBlock'].includes(this.c.interceptedProgram.programRing.type)
              && (value.id !== this.c.interceptedProgram.programRing.id)) {
              this.cloneBlockIterator();
              return true;
  ...
  findVarScopeIterateAllBlocks(programRing) {
    for (let i = 0; i < this.c.program.length; i++) {
      ...
      if (['startBlock', 'ifBlock', 'whileBlock', 'doWhileBlock', 'forBlock'].includes(type)) {
        const { scopeProp } = this.c.program[i].getScopeProps(type, 'NO');
        if (!Utils.isEmpty(programRing.code) && 
            this.c.program[i][scopeProp].hasKey(programRing.variableName) && 
            (this.c.program[i][scopeProp].get(programRing.variableName).id !== programRing.id) &&
            (this.c.program[i].id !== programRing.parentId && (this.c.program[i].id !== programRing.id))) {
          this.cloneBlockIterator();
          return true;
        }
        if (type === 'ifBlock') {
          if (!Utils.isEmpty(programRing.code) && 
              this.c.program[i].ifScopeBranchRight.hasKey(programRing.variableName) && 
              (this.c.program[i].id !== programRing.parentId) && (this.c.program[i].id !== programRing.id)) {
            this.cloneBlockIterator();
            return true;
          }
  ...
  checkParentBlockScope(programHook, arrowType) {
    if (this.innerBlocks.length > 0) {
      if (arrowType === 'out' && this.c.interceptedProgram.programRing.parentId === null) 
        return { isParentVarUsed: false };
      for (let i = 0; i < this.innerBlocks.length; i++) {
      const { type } = this.innerBlocks[i];
      if (this.checkIoBlockScope({ programRing: this.innerBlocks[i], programHook, arrowType }))
        return { isParentVarUsed: true, parentVarName: this.innerBlocks[i].variableName };
      }
    }
    return { isParentVarUsed: false };
  }
  ...
  removeAndUpdateVariablesScopeOnSwap({ programRing, programHook, arrowType }) {
    if (!programRing.parentId) {
      this.updateAndRemoveScopeBlockWithNotParent({ programRing, programHook, arrowType });
    } else if (programRing.parentId) {
      this.updateAndRemoveScopeBlockWithParent({ programRing, programHook, arrowType });
    }
  }

  updateAndRemoveScopeBlockWithNotParent({ programRing, programHook, arrowType }) {
    const value = this.c.program[0].globalScope.get(programRing.variableName);
    if(!value) return;
    if (['inner-out-yes', 'inner-out-no'].includes(arrowType)) {
      const indexHook = this.c.program.findIndex(block => block.id === programHook.id);
      const { scopeProp, scope } = this.c.program[indexHook].getScopeProps(this.c.program[indexHook].type, this.c.program[indexHook].blockProps.branch);
      value.scope = scope;
      this.c.program[indexHook][scopeProp].update(programRing.variableName, { ...value });
      this.removeScopeVar = { scope: 'globalScope', index: 0, varName: programRing.variableName };
    } else if (['bottom-inner-out-yes', 'bottom-inner-out-no'].includes(arrowType)) {
      const indexHookParent = this.c.program.findIndex(block => block.id === programHook.parentId);
      const { scopeProp, scope } = this.c.program[indexHookParent].getScopeProps(this.c.program[indexHookParent].type, this.c.program[indexHookParent].blockProps.branch);
      value.scope = scope;
      this.c.program[indexHookParent][scopeProp].update(programRing.variableName, { ...value });
      this.removeScopeVar = { scope: 'globalScope', index: 0, varName: programRing.variableName };
    }
  }
  
  updateAndRemoveScopeBlockWithParent({ programRing, programHook, arrowType }) {
    const indexParentHook = this.c.program.findIndex(block => block.id === programRing.parentId);
    const { scopeProp } = this.c.program[indexParentHook].getScopeProps(this.c.program[indexParentHook].type, programRing.blockProps.branch);
    const value = this.c.program[indexParentHook][scopeProp].get(programRing.variableName);
    if (!value) return;
    if (['inner-out-yes', 'inner-out-no'].includes(arrowType)) {
      const indexHook = this.c.program.findIndex(block => block.id === programHook.id);
      const branch = arrowType === 'inner-out-no' ? 'NO' : 'YES';
      const scopePropHook = programHook.getScopeProps(programHook.type, branch).scopeProp;
      const scopeHook = programHook.getScopeProps(programHook.type, branch).scope;
      value.scope = scopeHook;
      this.c.program[indexHook][scopePropHook].update(programRing.variableName, { ...value });
      this.removeScopeVar = { scope: scopeProp, index: indexParentHook, varName: programRing.variableName };
    } else if (['bottom-inner-out-yes', 'bottom-inner-out-no'].includes(arrowType)) {
      const indexHook = this.c.program.findIndex(block => block.id === programHook.parentId);
      const branch = arrowType === 'bottom-inner-out-no' ? 'NO' : 'YES'
      const scopePropHook = this.c.program[indexHook].getScopeProps(this.c.program[indexHook].type, branch).scopeProp;
      const scopeHook = programHook.getScopeProps(programHook.type, branch).scope;
      value.scope = scopeHook;
      this.c.program[indexHook][scopePropHook].update(programRing.variableName, { ...value });
      this.removeScopeVar = { scope: scopeProp, index: indexParentHook, varName: programRing.variableName };
      ...
      this.c.program[0].globalScope.update(programRing.variableName, { ...value });
      this.removeScopeVar = { scope: scopeProp, index: indexParentHook, varName: programRing.variableName };
    }
  }
  ...
  setScopeFromOutsideBlock({ programRing, programHook, arrowType }) {
    if (['defineBlock', 'inputBlock'].includes(programRing.type)) {
      if (programRing.type === 'inputBlock' && programRing.radioOption === 'selectVar') return;
      if (Utils.isEmpty(programRing.code)) return;
      const indexRing = this.c.program.findIndex(block => block.id === programRing.id);
      if (['inner-out-yes', 'inner-out-no'].includes(arrowType)) {
        const branch = 'inner-out-no' === arrowType ? 'NO' : 'YES';
        const indexHook = this.c.program.findIndex(block => block.id === programHook.id);
        const { scopeProp, scope } = this.c.program[indexHook].getScopeProps(this.c.program[indexHook].type, branch);
        this.c.program[indexRing].addVariableToScope({ index: indexHook, scopeProp, scope, isInFlow: true });
      } else if (['bottom-inner-out-yes', 'bottom-inner-out-no'].includes(arrowType)) {
        ...
        this.c.program[indexRing].addVariableToScope({ index: 0, scopeProp: 'globalScope', scope: 'Global scope', isInFlow: true });
      }
    }
  }
  ...
  widthManagerOutsideBlocks() {
    const nodes = this.c.program.filter(block => (
      ['forBlock', 'whileBlock', 'doWhileBlock', 'ifBlock'].includes(block.type)
    ...
    this.parentDeep.forEach(parent => {
      ...
      if (indexParent > -1) {
        if (['whileBlock', 'forBlock', 'doWhileBlock'].includes(type)) {
          ...
          const accum = !isRemoved ? incrementY :
            this.ioBlocks.includes(programRing.type) ? 
                    -programRing.blockProps.h : -programRing.blockProps.fullH;
          this.c.program[index].blockProps.fullH += accum;
        }
          
      }
      if (this.loopBlocks.includes(type)) this.updateHeightLoop(index, isRemoved);
    ...
    this.c.blockState.toArray()
      .forEach(block => {
        ...
        if(props.render) {
          ...
          this.c.program[index].move(this.c.program[index].x, this.c.program[index].y); 
        }
      });
    this.flowErrorManager.execute();
  }
  ...
  updateHeightLoop(index, isRemoved = false) {
    const plusOutHeight = this.c.program[index].accumulativeHeight > 0 ? this.c.program[index].accumulativeHeight + this.widthLine : this.widthLine;
    const plusHeight = this.c.program[index].accumulativeHeight > 0 ? this.c.program[index].accumulativeHeight : 0;
  ...
  updateHeightDoWhile(index, isRemoved = false) {
    ...
    if(isRemoved){
      height = -incrementY; 
    } else if (this.c.program[index].accumulativeHeight > this.doWhileDefaultAccum) {
      height = incrementY;
    }
  ...
  updateHeightIf(index, isRemoved = false) {
    const { incrementY, programRing: { id } } = this.c.interceptedProgram;
    ...
    let plusY = 0;
    const childrenBranchLeft = this.c.getSortedChildren(this.c.program[index].id, 'NO');
    const childrenBranchRight = this.c.getSortedChildren(this.c.program[index].id, 'YES');
    const { totalBranchL, totalBranchR } = this.totalHeightByBranch({ childrenBranchLeft, childrenBranchRight });
    ...
    this.c.program[index].updateHeight({ leftBranchHeight: totalBranchL, rightBranchHeight: totalBranchR, incrementOutLine, incrementHeightLeftLine, incrementHeightRightLine, incrementLeftRing, incrementRightRing, heightLeftLine, heightRightLine });
  }

  totalHeightByBranch({ childrenBranchLeft, childrenBranchRight }) {
    const totalBranchL = childrenBranchLeft
      .map(child => {
        const accum = child.type === 'doWhileBlock' ? this.doWhileDefaultAccum : 0;
  ...
  runCommand(ev) {
    if (ev.code === 'Enter') {
      const command = document.querySelector(`#${this.c.consolePromptElement}`).value;
      if (command.length) {
  ...
  removeBlockPropsOutsideFlow() {
    if (this.outsideFlowBlock) {
      if (this.outsideFlowBlock.hooks[0].occupied) {
        const index = this.c.program.findIndex(block => block.id === this.outsideFlowBlock.id)
  ...
  removeBlockOutsideFromGraph() {
    ...
    this.c.interceptedProgram = { programRing: blockDroped, incrementY: fullHeight, arrowType };
    const parentId = blockDroped.parentId;
    const blockState = this.c.blockState.get(blockDroped.id);
    const topBlockIndex = this.c.program.findIndex(block => block.id === blockState.lastHook);
    const bottomBlockNode = this.c.graph.getNodeEdges(blockDroped.id);
    const bottomBlockIndex = bottomBlockNode[0] ? this.c.program.findIndex(block => block.id === bottomBlockNode[0]) : -1;
    const topBlock = this.c.program[topBlockIndex];
    const bottomBlock = bottomBlockIndex > -1 ? this.c.program[bottomBlockIndex] : null;
    ...
    if (nodeEdges.length === 1) {
      ...
      if (['doWhileBlock', ...this.loopBlocks].includes(blockDroped.type)) {
        ...
        this.delReplacedEdgesOnRemove(blockDroped.id, [tempEdge, tempEdge2]);
        this.addReplacedEdgesOnRemove(blockDroped.id, [null, tempEdge, tempEdge2]);
      }
  ...
  removeBlockFromGraph(blockDroped, i) {
    ...
    if (blockDroped && blockDroped.hooks[0].occupied) {
      let fullHeight = this.getRemoveHeight(blockDroped, 0);
      this.c.interceptedProgram = { programRing: blockDroped, incrementY: fullHeight };
      const parentId = blockDroped.parentId;
      const blockState = this.c.blockState.get(blockDroped.id);
      const topBlockIndex = this.c.program.findIndex(block => block.id === blockState.lastHook);
      const bottomBlockNode = this.c.graph.getNodeEdges(blockDroped.id);
      ...
      this.updateParentHeight(parentId, true);
        ...
        if (blockDroped) {
          ...
          index = this.c.program.findIndex(block => block.id === blockDroped.id);
        }
  ...
  cloneBlockIterator() {
    this.cloneBlocks.forEach((cloneBlock,index) => {
      ...
      if (index !== 0) {
        this.c.program[cloneBlock.index].isLocked = this.cloneBlocks[0].hooks[0].occupied ? false : true;
      }
    });  
  }
  ...
  dialogConfirm(text) {
    return new Promise(resolve => {
      ...
      document.querySelector("body").insertBefore(wrapper, document.querySelector("body").childNodes[0]);
      const onClick = pass => {
        resolve(pass);
        this.c.removeDialogBox = document.querySelector('#removeBlockDialogBox').checked;
        wrapper.remove();
      };
  ...
  removeTab(blockDroped) {
    if (blockDroped.type === 'wrapBlock' && blockDroped.wrapType === 'function_declaration') {
  ...
  isVarNameInUse(blockDroped, i) {
    ...
    if (['defineBlock', 'inputBlock', 'forBlock'].includes(blockDroped.type)) {
      ...
      if (!blockDroped.parentId) {
        const blocks = this.c.program.filter(block => (!['startBlock', 'endBlock'].includes(block.type) && block.hooks[0].occupied && block.id !== blockDroped.id));
        return blocks.some(block => regex.test(block.code));
      } else {
        const indexParent = this.c.program.findIndex(block => block.id === blockDroped.parentId)
        if (indexParent < 0) return false;
        const children = this.c.getChildren(this.c.program[indexParent].id).filter(child => child.id !== id);
  ...
  getRemoveHeight(block, accum) {
    const { accumulativeHeight, type, blockProps: { h } } = block;
    let fullHeight = accum;
    fullHeight += type !== 'doWhileBlock' ? 
  ...
  removeInnerBlocks(removeBlocks, lessI) {
    for (let [value] of Object.entries(removeBlocks)) {
      let indexProgram = this.c.program.findIndex(block => block.id === value);
  ...
  replaceNodeEdgesOnRemove(idTopBlock, removeNodeId, isRemoved = true) {
    ...
    if (length === 1) {
      ...
      if (this.c.blockState.hasKey(nodeEdges[0])) {
        ...
        this.c.blockState.update(nodeEdges[0], { arrowHook: lastHook.arrowHook });
      }
      if (removedIdx == 1) {
        this.c.graph.removeEdge(idTopBlock, removeNodeId);
      ...
      if (removedIdx == 2) {
        this.delReplacedEdgesOnRemove(idTopBlock, [removeNodeId, edges[0], edges[2]]);
        edgesToAdd.push(edges[0]);
        edgesToAdd.push(edges[1]);
        if (nodeEdges.length > 0 && nodeEdges[0] !== null && nodeEdges[0] !== undefined) {
          this.c.graph.removeEdge(removeNodeId, nodeEdges[0]);
          edgesToAdd.push(nodeEdges[0]);
        } else {
        ...
        this.delReplacedEdgesOnRemove(idTopBlock, [removeNodeId, edges[1], edges[2]]);
    ...
    if (nodeEdges.length > 0) {
      if(edgesBottom)
      this.c.graph.nodes.set(nodeEdges[0], edgesBottom);
    }
  }
  ...
  deleteJavaStateVars(block) {
    switch (block.type) {
      case 'defineBlock':
        ...
        break;
      case 'inputBlock':
        const inputsBlocks = this.c.program.filter(prg => prg.type === 'inputBlock');
      ...
      case 'codeBlock':
        if (block.isImport) {
          block.delImportStatement(`${block.imports[block.importValue -1].value}\n`);
        }
        break;
      default:
        break;
  ...
  removeVariablesFromState(block) {
    if (!block.parentId) {
      ...
      const index = this.c.program.findIndex(parentBlock => parentBlock.id === block.parentId);
      const { type } = this.c.program[index];
      const { scopeProp } = this.c.program[index].getScopeProps(type, block.branch);
  ...
  updateVariablesFromState(block, isInFlow) {
    ...
    if (block.type === 'forBlock' && block.radioVar === 'defineVar') {
      ...
      const index = this.c.program.findIndex(parentBlock => parentBlock.id === block.parentId);
      const { type } = this.c.program[index];
      const { scopeProp } = this.c.program[index].getScopeProps(type, block.blockProps.branch);
  ...
  showContextMenu(ev) {
    ...
    if (ev.ctrlKey) {
      ...
      if (this.c.selectBlock) {
        if (this.c.selectBlock.isPointInRectangle(x, y) && this.c.selectedTab != '1') {
          const selectContextMenu = new SelectContextMenu(this.c, 'selection');
          selectContextMenu.filterMenu('edition', -1).show(ev,{ x, y, selection: this.c.selectBlock });
        } else {
          this.c.ungrabedUi();
          this.c.updateCanvas();
        }
        ...
        if (indexBlock > -1) {
          this.isEditing = true;
          const selectContextMenu = new SelectContextMenu(this.c, 'edition');
          selectContextMenu.filterMenu('selection', indexBlock).show(ev,{ x, y });
          this.c.ungrabedUi();
        } else {
          this.c.ungrabedUi();
          this.c.updateCanvas();
