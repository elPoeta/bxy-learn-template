class FlowAPI {
  ...
  getBFS(graph) {
    this.tree = graph.breadthFirstSearch(this.c.startBlockId).filter(g => g !== this.c.endBlockId);
    return this;
  }
  ...
  getApiBlockProps(blocks, blockState, apiId) {
    ...
    this.tree.forEach(node => {
      ...
      if(index > -1) {
        programs.push(this.getStateBlock(blocks[index], state, this.c.languageOutput ));   
      }
    });
    ...
    this.api.program[apiId] = {
            ...
            blocksOutside,
            tab: {
              name: !this.c.tabs.length ? 'main' : this.c.tabs[this.c.selectedTab].name,
              type: !this.c.tabs.length ? 'main' : this.c.tabs[this.c.selectedTab].type
            }         
    }
    return this;
  }
  ...
  saveAllApiTabs(tabId) {
    this.c.tabs.forEach(tab => {
      if(tab.id != tabId) {
        const newTabBlocks = this.deleteTabProps(tab.api.program[tab.id].blocks);
  ...
  getBlocksOutside(blocks, blockState, apiId) {
    ...
    outBlocks.forEach(block => {
        const tree = this.c.graph.breadthFirstSearch(block.id);
          ...
          if(index > -1) {
            const state = { };
            const stateBlock = this.getStateBlock(blocks[index], state, this.c.languageOutput);
  ...
  getVarPropsByLanguage(language, type) {
    ...
    case "java":
      return type === 'defineBlock' ? 
              ['declaration','declarationType', 'arraySizeOne', 'arraySizeTwo', 'variableName', 'variableValue'] :
             type === 'inputBlock' ?  ['declaration','declarationType', 'variableName', 'variableValue', 'radioOption','userInputEnabled', 'externalVariableId'] :   
             type === 'outputBlock' ? ['expression', 'radioOption'] :
             type === 'codeBlock' ? ['importValue', 'isImport'] :
             type ==='forBlock' ? ['initialValue', 'radioVar', 'radioOption', 'variableName', 'variableValue', 'variableCompareValue', 'operation'] :
             type === 'wrapBlock' ?  ['wrapType', 'tabId', 'bgColor', 'lineColor', 'fontColor', 'params', 'functionName', 'returnFunctionType', 'returnType'] : []; 
    case "javascript":
      return  type === 'defineBlock' ? ['declaration','declarationType', 'arraySizeOne', 'arraySizeTwo', 'variableName', 'variableValue'] :
              type === 'inputBlock' ?  ['declaration','declarationType', 'variableName', 'variableValue', 'radioOption', 'operation', 'expression'] :   
              type === 'outputBlock' ? ['expression', 'radioOption'] :
              type === 'forBlock' ? ['initialValue', 'radioVar', 'radioOption', 'variableName', 'variableValue', 'variableCompareValue'] :
              type === 'wrapBlock' ? ['wrapType', 'tabId', 'bgColor', 'lineColor', 'fontColor', 'params', 'functionName', 'returnType'] : [];  
    default:
      return [];
    }
      ...
      for (const [key, value] of Object.entries(program[k])) {
        if(key === 'blocks') {
          value.forEach(props => {
            ...
            const hookArrowIndex = this.getIndexHook(this.c.program[indexHook].type, hook);
            ...
            this.c.program[indexRing].compileError = (this.isFromTab && props.compileError) ? props.compileError : { hasError: false, errorMessages: [] };
            this.c.program[indexRing].colors = (this.isFromTab && props.colors) ? props.colors : null;
  ...
  newFlowchart({ projectCompositeId, projectName, isUndoRedo }) {
    ...
    const code = this.c.editor !== null ? this.c.editor.session.getValue() : null;
    ...
    if(this.isFromTab || isUndoRedo) {
      ...
      if(code) {
        this.c.log(`<div id="innerFlowchartIdeEditor"></div>`, 'flowchartIdeEditor');
        this.c.renderAceEditor({ code, selectorId: 'innerFlowchartIdeEditor', editor: 'editor' });
        if(errors) {    
  ...
  createNewProgram(id, type, x, y, w, h, code, wrapType) {
    this.c.program.push(BlockFactory.getImplementation(id, this.c, type, x, y, w, h, code, true, this.c.languageOutput, wrapType));
  }
  ...
  getIndexHook(typeBlock, typeArrow) {
    ...
    switch (typeBlock) {
      ...
      case 'ifBlock':
        return types.includes(typeArrow) ? 9 : typeArrow === 'inner-out-yes' ? 4 : 3;
      default:
        return 0;
    }
  }
  ...
  setTabs() {
    ...
    for (const [k, val] of Object.entries(program)) {
      if(k != 0 && k != 1) {
        ...
        this.c.tabs.push({ id: k, name: program[k].tab.name, type: program[k].tab.type, api, scale: 1, xScroll: 0, yScroll: 0, yScrollPalette:0, isOpened: false, selected: false  });
      } else {
  ...
  setPalette(index) {
    if(Utils.isEmpty(this.api)) return;
    const paletteTab = this.api.program[index].tab.palette ||  this.c.paletteManager.getAllBlocks();
    ...
    for(let j = 0; j < this.c.program.length; j++) {
      ...
      if (!Utils.isEmpty(blk.code)) {
        if ('codeBlock' === blk.type && blk.isImport) {
          ...
          if ('defineBlock' === blk.type || ('inputBlock' === blk.type && (blk.radioOption === 'defineVar' || blk.userInputEnabled))) {
            this.addVariableToScope({ i, j, scopeProp: 'globalScope', scope: this.c.scopeType.GLOBAL_SCOPE, isInFlow: true });
            if (Utils.isEmpty(blk.variableValue) || ('inputBlock' === blk.type && blk.userInputEnabled)) {
              this.c.program[i].importsAndInstancesState.update('scanner',{ name: 'scanner', value: 'new Scanner(System.in)', declaration: 'Scanner', declarationType: 'Class', used: false });
            }
          } 
        }
      }
  ...
  setForScope(i) {
    if (this.c.program[i].radioVar === 'defineVar') {
      this.addVariableToScope({ i, j: i, scopeProp: 'forScope', scope: this.c.scopeType.FOR_SCOPE, isInFlow: true });
    }
  ...
  setLoopScope(i, scopeProp) {
    ...
    const scope = block.type === 'whileBlock' ? this.c.scopeType.WHILE_SCOPE : 
      block.type === 'forBlock' ? this.c.scopeType.FOR_SCOPE : this.c.scopeType.DO_WHILE_SCOPE;
  ...
  iterateChildrenScope(i, scope, scopeProp, children) {
    children.forEach(child => {
      ...
      if ('defineBlock' === type || ('inputBlock' === type && (this.c.program[j].radioOption === 'defineVar' || this.c.program[j].userInputEnabled))) {
        this.addVariableToScope({ i, j, scopeProp, scope, isInFlow: true });
        if (Utils.isEmpty(this.c.program[j].variableValue)  || ('inputBlock' === type && this.c.program[j].userInputEnabled)) {
          this.c.program[0].importsAndInstancesState.update('scanner', { name: 'scanner', value: 'new Scanner(System.in)', declaration: 'Scanner', declarationType: 'Class', used: false });
        }
      }
    });
  }
  ...
  setImports(i, impVal) {
    const importVarState = this.c.program[i].importsAndInstancesState.get('import');
    const value = !importVarState ? new Set([impVal]) : new Set([...importVarState.value, impVal]);
    this.c.program[i].importsAndInstancesState.update('import', { name: 'import', value, declarationType: 'import' });
  }
  ...
  isJsonFile({ projectCompositeId, projectName }) {
    const executeGet = data => this.getJsonFileContent({ projectCompositeId, projectName, data });
    const executeCreate = () => this.createJsonFile({ projectCompositeId, projectName });
    this.jsonFileManager.isJsonFile({ projectCompositeId, executeGet, executeCreate, isLoadContent: true });
  }
  ...
  getJsonFileContent({ projectCompositeId, projectName, data }) {
    ...
    this.setPalette("0");
    this.createFlowFromAPI({ key: "0", projectCompositeId, projectName, isUndoRedo: false });
    this.setTabs();
    this.c.centerStartBlock();
  }
  
  createJsonFile({ projectCompositeId, projectName }) {
    const content = this.jsonFileManager.getFlowchartJsonContent(this.c.languageOutput);
    this.saveJsonFile({ projectCompositeId, content },() => {  
      browxyStartUp.fileOperation.renameOpenedFile(`${projectCompositeId}/flowchart.json`, `${projectCompositeId}/flowchart.json`, 'flowchart.json');
  ...
  downloadJSONFile() {
    ...
    .saveAllApiTabs(tabId);
    this.jsonFileManager.createDownloadLink(`${this.c.flowchartName.toLowerCase()}_${new Date().toISOString()}.json`, JSON.stringify(this.api, null, 2)).click();
  }
